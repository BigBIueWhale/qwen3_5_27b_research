...
3. ~~**Boolean/None capitalization**~~ — **DONE (2026-04-03).** Added `case bool:` returning `"True"`/`"False"`, changed nil to return `"None"`, added `case float64:` with `strconv.FormatInt` for integer-valued floats. `TestFormatToolCallArgumentMatchesOfficialTemplate` 11/11 PASS, `TestQwen35RendererUsesXMLToolCallingFormat` 2/2 PASS.
...
- `case float64:`: integer-valued floats use `strconv.FormatInt(int64(v), 10)` instead of `fmt.Sprintf("%v", v)`, producing `"10000000000"` instead of `"1e+10"` (matching Python `str(10000000000)`)
...
**Test status (2026-04-03):** Two tests enforce the correct behavior and both PASS: (1) `TestFormatToolCallArgumentMatchesOfficialTemplate` in `qwen3coder_test.go` (unit test, 11/11 cases PASS — was 7/11 before the fix), (2) `TestQwen35RendererUsesXMLToolCallingFormat` in `qwen35_test.go` (integration test, 2/2 subtests PASS — was 0/2 before the fix). The dead `TestFormatToolCallArgumentThinkingVL` (which tested `formatToolCallArgument` from a VL test file despite VL never calling it, with wrong expected values) was deleted on 2026-04-01.
