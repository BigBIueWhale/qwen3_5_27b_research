...
Note: `TestQwen35RendererToolDefinitionsMatchOfficialTemplate` (updated 2026-04-01) now has two tools with byte-exact HF ground truth from `apply_chat_template`. Tool 1 (`get_weather`, `location: str, filters: list[str]`) exercises HTML escaping, field ordering, and single-key items — **passes**. Tool 2 (`choose_color`, `color: Literal["red", "green", "blue"]`) exercises enum/description field ordering — **FAILS by design** because Go's `ToolProperty` struct declares `Description` before `Enum`, but HF's `get_json_schema()` inserts `enum` first and HF's `tojson` override preserves insertion order (`sort_keys=False`). Stock Jinja2's `sort_keys=True` would agree with Go's wrong order (d < e alphabetically), but the model was trained with HF's override — verified empirically. The real multi-key `items` sub-objects come from complex element types via `_parse_type_hint()` recursion (e.g., `list[dict[str, int]]` → items = `{"type": "object", "additionalProperties": {"type": "integer"}}`) — these are tested by the proposed `TestQwen35ToolPropertyRoundTripFidelity` (cases 4-5), which arrives with the `ToolProperty` struct rewrite.
...
3. ~~**Boolean/None capitalization**~~ — **DONE (2026-04-03).** Added `case bool:` returning `"True"`/`"False"`, changed nil to return `"None"`, added `case float64:` with `strconv.FormatInt` for integer-valued floats. `TestFormatToolCallArgumentMatchesOfficialTemplate` 14 cases (13 PASS, 1 FAILS — `very_large_int_float`: `float64(1e20)` overflows `int64`, producing garbage instead of `"1e+20"`; fix: add range check before `int64` conversion). `TestQwen35RendererUsesXMLToolCallingFormat` 2/2 PASS. `AssistantPrefillWithThinking/think_switch_with_tool_history` exercises all four scalar paths (string, bool, int, nil) in a rendered tool history context.
4. **Object key ordering** — The `parseValue()` function in `model/parsers/qwen3coder.go` uses `json.Unmarshal` into `map[string]any` for object argument values, destroying key ordering. The renderer then alphabetizes via `json.Encoder`. Fix requires an insertion-order-preserving JSON parser (the same approach llama.cpp uses via `nlohmann::ordered_json`). Three KV cache round-trip tests (cases 10, 11, 15) and two number normalization tests (cases 12, 13) already exist and correctly fail, documenting the bug.

For Qwen 3.5 specifically, the current state is: (1a) `TestQwen35RendererToolDefinitionsMatchOfficialTemplate` Tool 1 (`get_weather` with `list[str]`) **passes** — byte-exact against HF ground truth. (1b) Tool 2 (`choose_color` with `Literal["red", "green", "blue"]`) **FAILS by design** — enforces `enum`/`description` field ordering against HF ground truth (added 2026-04-01). (2) The new `TestQwen35ToolPropertyRoundTripFidelity` (not yet implemented) must pass all 5 cases — `nullable` field preserved at property level, `additionalProperties` field preserved at property level, `prefixItems` field preserved at property level, `nullable` key ordering preserved inside items sub-objects for `list[Optional[int]]`, and `additionalProperties` key ordering preserved inside items sub-objects for `list[dict[str, int]]`. (3) The full round-trip consistency test in `server/qwen35_kvcache_roundtrip_test.go` must pass completely — including the 5 currently-failing cases for object key ordering and number normalization.
