...
**Date:** 2026-03-13 (updated 2026-03-22: Section 4.2 corrected with verified HuggingFace Transformers ground truth — exact `ToolProperty` struct layout specified with recursive types (`Items *ToolProperty`, not `any`) for correct key ordering at all nesting levels; `items` ordering confirmed as real issue for complex element types like `list[dict[str,T]]` and `list[Optional[T]]` (empirically verified via `get_json_schema()` + `apply_chat_template` on the official Qwen 3.5 tokenizer — see the `prove_multikey_items.py` verification script); blast radius table added; new Section 4.4 for `formatToolCallArgument` scalar divergences; action items updated) (updated 2026-04-01: Section 4.2 note updated — `TestQwen35RendererToolDefinitionsMatchOfficialTemplate` rewritten with byte-exact HF ground truth, old out-of-training-distribution `items` example replaced with realistic `list[str]` single-key items; `TestQwen35RendererBackToBackToolCallsAndResponses` upgraded to byte-exact with `think=true`/`think=false` subtests — completes the think=false coverage matrix across all `lastQueryIndex` positions; action items table updated; `TestQwen35RendererUsesXMLToolCallingFormat` upgraded to byte-exact with boolean argument that enforces Gap 10 `true`→`True` divergence — test FAILS by design; `formatToolCallArgument` P2 action item marked test-enforced; `TestFormatToolCallArgumentMatchesOfficialTemplate` replaces old unit test with 11-case template ground truth verified against both Qwen 3.5 and Qwen3-Coder templates; dead `TestFormatToolCallArgumentThinkingVL` deleted — tested a function VL doesn't call; Section 4.4 updated with exact production callers, VL renderer path clarification, and test enforcement status; remaining issues table updated) (updated 2026-04-02: `TestQwen35RendererInterleavedThinkingAndTools` upgraded to byte-exact `got != want` comparison with official Jinja2 template ground truth — both think=true (1981 bytes) and think=false (1992 bytes) verified byte-identical between Go renderer output and Python template output via diff; this was the last renderer test using `strings.Contains` for its main assertions; all 9 renderer tests (20 subtests) now use full byte-exact comparison; `TestQwen35RendererNoThinkPrefill` also upgraded from `strings.HasSuffix` to `got != want` (74 bytes)) (updated 2026-04-03: Section 4.4 **DONE** — `formatToolCallArgument` in `model/renderers/qwen3coder.go` rewritten to match Python `str()`: `nil`→`"None"`, `case bool:`→`"True"`/`"False"`, `case float64:` integer-valued→`strconv.FormatInt`; `TestFormatToolCallArgumentMatchesOfficialTemplate` now 11/11 PASS; `TestQwen35RendererUsesXMLToolCallingFormat` now 2/2 PASS; sole remaining renderer test failure is `TestQwen35RendererToolDefinitionsMatchOfficialTemplate` Tool 2 enum/description ordering (P2 ToolProperty struct rewrite); scorecard, Section 4.2 header, Section 4.4, action items table, remaining issues table all updated) (updated 2026-04-03: `ToolDefinitionsMatchOfficialTemplate` expanded — Tool 3 (`search_records`, `list[dict[str, int]]`) FAILS on multi-key items key ordering; 3 `silent_field_loss` subtests FAIL on `nullable`/`additionalProperties`/`prefixItems` dropped during `json.Unmarshal`; `AssistantPrefillWithThinking` expanded — 3 `path_equivalence` subtests verify all three reasoning encodings produce byte-identical renderer output; total: 10 functions, 59 subtests; Section 4.2, scorecard, coverage matrix updated; `AssistantToolCallIsNotPrefill` restructured — both subtests now share same messages (with Thinking field) and `wantHistory` constant, properly enforcing P1; stale Gap 10 comments cleaned from test file)
...
**Model:** Qwen 3.5 27B (Alibaba Qwen), a hybrid recurrent architecture (`qwen3next`) combining GatedDeltaNet recurrent layers with standard full-attention layers. Running via Unsloth Dynamic 2.0 `UD-Q4_K_XL` GGUF quantization (17.6 GB, 851 tensors, text-only, locally downloaded)
...
**llama.cpp (post-refactor):** Two additional clones were verified at commits `0cd4f47` and `aa2d278` (both March 10, 2026). These contain a significant refactoring of the chat template handling: the per-model specialized handlers (`common_chat_params_init_qwen3_coder`, `common_chat_params_init_qwen3_vl`, etc.) have been replaced with a unified autoparser path in `common_chat_template_direct_apply()`. This refactoring fixes the `enable_thinking` generation prompt bug described in Section 7.1 — the unified path correctly passes `enable_thinking` to the Jinja template context, so the template's own `add_generation_prompt` block produces the correct 6-token non-thinking prefill. See Section 7.1 for details.
...
| JSON tool serialization | Fixed: shared tool-definition HTML-escaping end-to-end + Qwen-local `formatToolCallArgument` spaced JSON + `required`/`properties` ordering + `formatToolCallArgument` scalar bool/None/large-int capitalization (2026-04-03). Remaining open (all now test-enforced, FAILING by design): `ToolProperty` `enum`/`description` ordering, `ToolProperty` field loss (`nullable`, `additionalProperties`, `prefixItems`), recursive nested key ordering for complex element types (e.g., `list[dict[str, int]]` produces 2-key items sub-objects that Go alphabetizes — fix: `Items *ToolProperty` instead of `any`). For simple tools (no enum, no Optional, no complex element types), the fork produces **byte-identical** output to HuggingFace Transformers — verified by saving both outputs and running `diff`. Tool call arguments with scalar booleans now also match (produces `True`/`False`/`None` matching Python `str()`). | Full original bug set (HTML escaping, compact separators, key ordering, field loss, wrong bool/None capitalization) | Correct — Jinja2 `tojson` with no HTML escaping, spaced separators, preserved dict insertion order | **Fork** (fork > upstream; llama.cpp > both) |
| `enable_thinking` generation prompt | Correct via `emitEmptyThinkOnNoThink` — prefills `<think>\n\n</think>\n\n` when `think: false` | Same (correct) | **Fixed in post-refactor versions** (`0cd4f47`+): unified `common_chat_template_direct_apply()` passes `enable_thinking` in the unified path in `chat.cpp`, so the template produces the correct 6-token prefill. **Broken in versions up to `c5a7788`**: the old per-model handler `common_chat_params_init_qwen3_coder` in `chat.cpp` did not pass `enable_thinking` to Jinja; the old post-processing code in `chat.cpp` produced `<think>\n</think>` (3 tokens) instead of the official `<think>\n\n</think>\n\n` (6 tokens) — a token sequence never seen in training | **Fork** and **Upstream Ollama** (for older llama.cpp); **Tie** (for post-refactor llama.cpp) |
...
| Model-specific sampler defaults | Not implemented | Not implemented | Not implemented (llama.cpp's hardcoded sampling defaults in `common.h` — same defaults for all models) | Tie (all wrong) |
| Float precision in `tojson` | N/A (Go renderer) | N/A (Go renderer) | Minor flaw — 6 significant digits via C++ `ostringstream` versus Python's ~17 (in llama.cpp's `tojson` float serialization in `value.cpp`); also `1.0` → `1` | N/A |
...
**llama.cpp** (`d969e933` through `a0ed91a`) has the same behavior. The server's `init_sampler()` in llama.cpp's prompt token feeding in `server-context.cpp` loops through ALL prompt tokens and calls `common_sampler_accept(smpl.get(), id, false)` on each one. The penalty sampler in `llama-sampler.cpp` uses a ring buffer that makes no distinction between prompt-origin and generation-origin tokens. The penalty sampler in `llama-sampler.cpp` is **byte-identical** between `d969e933` and the latest `a0ed91a`. Defaults unchanged: `penalty_repeat=1.0` (disabled), `penalty_last_n=64`.
...
**llama.cpp's ring buffer architecture** (`d969e933`, the `ring_buffer` template in `llama-sampler.cpp`): Uses a proper `ring_buffer<llama_token>` template class with the same design — fixed capacity, O(1) push, modular arithmetic. The key difference: llama.cpp's `llama_sampler_penalties_accept()` is a **public** function called by external code (the server feeds prompt tokens through it), while the fork's `recordToken()` is **private** and called only from `Sample()`.
...
Note: `ssm.v_head_reordered` and `rope.mrope_interleaved` are **ollama-internal conventions**. They do not exist in llama.cpp's GGUF constants. llama.cpp sidesteps the problem entirely: RoPE type is hardcoded per architecture in `llama_model_rope_type()`, and V-head reordering is done **physically during GGUF conversion** by `_LinearAttentionVReorderBase` in `convert_hf_to_gguf.py` — the converter permutes tensor weights so no runtime flag is needed. Third-party GGUFs from Unsloth, bartowski, or llama.cpp's converter will **never** contain these keys.
...
The previous approach of routing `"qwen3.5"` through `Qwen3CoderRenderer`/`Qwen3CoderParser` caused seven distinct template fidelity problems (sections 2.1–2.7 below), **all now resolved**. All existing tests pass, and 21 new tests (9 renderer + 11 parser + 1 integration routing test in `qwen3_test.go`) cover the new code.
...
- The parser is **byte-for-byte identical** to upstream Ollama's implementation. 11 test functions cover all edge cases.
...
**Design decision (2026-04-03):** The fork should be at least as strict as the official template, and stricter where the template is too permissive. The template silently produces `<function=>` for empty tool function names — a token sequence absent from all training data. The fork should reject this with an error. Being more strict than the template is the right default when the alternative is silent corruption. Verified that adding all 6 validation checks to `Render()` would not break any existing test — all 59 byte-exact subtests use valid message arrays.
...
- Side-by-side comparison with llama.cpp's validation (including the corrected HTTP status code mapping — `raise_exception()` maps to HTTP 400 via `std::invalid_argument` wrapping in chat.cpp's `raise_exception()` mapping, not HTTP 500)
- The architectural solution: 6 one-liner validation checks placed inside `Render()` at the exact code points where the relevant state is already computed, eliminating all code duplication (5 matching the template's rejections + 1 stricter: empty function name)
- A complete validation checklist with what to add, what NOT to add (and why), and where NOT to add it
- Connection to `test_gap_analysis.md` Gap 11 — the edge cases become error-path tests once validation is implemented

---

## Part 4: JSON Serialization Mismatches in Tool Definitions and Tool Call Arguments

The fork (and upstream Ollama) serialize tool definitions differently from the Qwen 3.5 model's training data. The official Qwen 3.5 Jinja2 template uses the `tojson` Jinja2 filter. In HuggingFace Transformers (`transformers` @ `3a3b59c`), this filter is overridden (in HuggingFace Transformers' `tojson` override in `chat_template_utils.py`) to use `json.dumps` with default separators (`', '` and `': '`), no HTML escaping, no key sorting, and insertion order preservation. This is registered in `_cached_compile_jinja_template()` as `jinja_env.filters["tojson"] = tojson`. When the Qwen 3.5 template calls `{{ tool | tojson }}`, all arguments use defaults: `ensure_ascii=False`, `sort_keys=False`, `separators=None` (which in Python 3 defaults to `(', ', ': ')`).
...
### 4.2 Sub-Bug B — Key Ordering and Field Loss in Tool Definition Serialization — PARTIALLY DONE (`required`/`properties` fixed; `enum`/`description` ordering and field loss for `nullable`/`additionalProperties`/`prefixItems` remain OPEN; scalar boolean/None tool-call-argument capitalization DONE in Section 4.4)
...
| `ToolProperty.Items` (as `map[string]any`) | alphabetical (e.g., `additionalProperties`, `type`) | insertion order (e.g., `type`, `additionalProperties`) | **No — multi-key for complex element types, Go alphabetizes.** `list[dict[str,int]]` → items = `{"type": "object", "additionalProperties": {"type": "integer"}}` (2 keys). `list[Optional[int]]` → items = `{"type": "integer", "nullable": true}` (2 keys). Only basic element types like `list[str]` produce single-key items `{"type": "string"}`. Fix: change `Items any` to `Items *ToolProperty` so struct field ordering applies recursively. Empirically verified via the `prove_multikey_items.py` verification script |
...
The `ToolProperty` enum/description mismatch arises because `_parse_type_hint()` in `chat_template_utils.py` constructs `{"type": ..., "enum": [...]}` first, then `get_json_schema()` appends `schema["description"] = desc` last (where `get_json_schema()` adds the description field). Python dicts preserve insertion order, so the training data has `enum` before `description`. Go's `ToolProperty` struct declares `Description` before `Enum`, producing the opposite order. (The same ordering holds when `enum` comes from a docstring's `(choices: ...)` block instead of a `Literal` type hint — `get_json_schema()` inserts the enum field before it inserts the description field, producing the same `enum`-before-`description` order.)
...
The field loss rows (`Optional[T]`, `Dict[str, T]`, `Tuple[T1, T2]`) arise because Go's `ToolProperty` struct (in `api/types.go`) has six fields (`anyOf`, `type`, `items`, `description`, `enum`, `properties`) but HuggingFace Transformers' `_parse_type_hint()` can produce three additional fields: `nullable` (for `Optional[T]` / `Union[T, None]`), `additionalProperties` (for `Dict[str, T]`), and `prefixItems` (for `Tuple[T1, T2]`). Go's `json.Unmarshal` silently drops unknown fields, so these constraints vanish during the client→Go→renderer round-trip.

**Why this is a real problem, not just a cosmetic difference:** Unlike the `"return"` field that HuggingFace's `get_json_schema()` also produces (which is informational metadata the model was trained to see or not see — both variants are in-distribution), these three fields **change the semantic meaning** of the tool parameter schema. `{"type": "integer", "nullable": true}` means "this parameter accepts an integer or null" — it is a fundamentally different schema from `{"type": "integer"}` which means "this parameter must be an integer." The model was trained on both schemas, but they mean different things and produce different model behavior. When Ollama drops `"nullable": true`, the model sees a schema that says "integer required" when the client declared "integer or null allowed." The model may then refuse to pass `null` for that parameter, or produce a tool call that fails because the downstream function expected a nullable argument.

**Why the user cannot avoid this:** If a Python developer writes `def set_age(name: str, age: Optional[int] = None)` and uses HuggingFace's `get_json_schema()` to generate the tool definition, `"nullable": true` is automatically included in the JSON. This is the standard, documented way to create tool definitions for HuggingFace-hosted models. The developer sends this JSON to Ollama's API, and Ollama silently destroys the `"nullable": true` field during `json.Unmarshal` because Go's `ToolProperty` struct has no `Nullable` field. The developer has no way to know this happened — no error, no warning. The tool definition arrives at the renderer with the constraint removed.

Of the three lost fields, `nullable` is the most impactful — `Optional[T]` parameters are extremely common in Python function signatures. `additionalProperties` (from `dict[str, T]`) and `prefixItems` (from `tuple[T1, T2]`) are less common but follow the same pattern: the field carries type constraint information that changes the schema's meaning, and dropping it produces a different schema that the model interprets differently.
...
**Why `*ToolProperty` instead of `any` for `Items`, `AdditionalProperties`, and `PrefixItems`:** These fields hold nested JSON Schema objects that follow the same structure as `ToolProperty` itself. If they were `any`, Go's `json.Unmarshal` would parse them into `map[string]any`, and `json.Marshal` would alphabetize the keys — producing wrong ordering for complex element types. Using `*ToolProperty` (or `[]ToolProperty` for `PrefixItems`) means the nested schemas go through the same struct with the same field declaration order, so `json.Marshal` produces correct key ordering **at every nesting level recursively**. This was empirically verified by running `get_json_schema()` on `list[dict[str, int]]` and `list[Optional[int]]` through the official `Qwen/Qwen3.5-27B` tokenizer (verified via the `prove_multikey_items.py` script):

- `list[dict[str, int]]` → items = `{"type": "object", "additionalProperties": {"type": "integer"}}` — 2 keys, HF order is `type` then `additionalProperties`
- `list[Optional[int]]` → items = `{"type": "integer", "nullable": true}` — 2 keys, HF order is `type` then `nullable`
...
Note: `TestQwen35RendererToolDefinitionsMatchOfficialTemplate` (updated 2026-04-03) now has three tools and three silent field loss subtests, all with byte-exact HF ground truth from `apply_chat_template`. Tool 1 (`get_weather`, `location: str, filters: list[str]`) exercises HTML escaping, field ordering, and single-key items — **passes**. Tool 2 (`choose_color`, `color: Literal["red", "green", "blue"]`) exercises enum/description field ordering — **FAILS by design**. Tool 3 (`search_records`, `filters: list[dict[str, int]]`) exercises multi-key `items` sub-object key ordering — **FAILS by design** (Go's `Items any` → `map[string]any` → alphabetizes keys; HF preserves insertion order: `type` before `additionalProperties`). Three `silent_field_loss` subtests exercise field preservation through the JSON unmarshal→render round-trip (the real client path): `nullable` from `Optional[str]`, `additionalProperties` from `dict[str, int]`, `prefixItems` from `tuple[int, int]` — all **FAIL by design** because `ToolProperty` lacks these fields. Each accepts either correct rendering or an error; fails only on silent corruption. All known tool definition divergences now have test enforcement.
...
Verified examples at the **items level**: `list[str]` → items = `{"type": "string"}` (1 key, no ordering issue) ✓; `list[dict[str, int]]` → items = `{"type": "object", "additionalProperties": {"type": "integer"}}` (2 keys, `type` before `additionalProperties`) ✓; `list[Optional[int]]` → items = `{"type": "integer", "nullable": true}` (2 keys, `type` before `nullable`) ✓.
...
Every verified model (Qwen 3.5, Qwen3VL, OLMo3, OLMo3.1, and the GLM-4 family) uses HuggingFace Transformers' insertion-order `tojson` override, which produces `enum` before `description`. No verified model uses stock Jinja2's `sort_keys=True`. The HuggingFace `tojson` override is hardcoded globally in `_cached_compile_jinja_template()` at `_cached_compile_jinja_template()` in `chat_template_utils.py` — there is no per-model opt-out, and no model in the HF ecosystem was trained with `sort_keys=True`. llama.cpp confirms this from the other side: its Jinja engine explicitly `throw not_implemented_exception("tojson sort_keys=true not implemented")` and uses `as_ordered_object()` with the comment `"IMPORTANT: need to keep exact order"` in llama.cpp's `as_ordered_object()` in `value.cpp`. **Insertion order is the universal training format for every tool-calling model.**

The `Items` ordering IS an issue for complex element types. HuggingFace Transformers' `get_json_schema()` produces multi-key `items` sub-objects when the element type is complex: `list[dict[str, int]]` → items = `{"type": "object", "additionalProperties": {"type": "integer"}}` (2 keys), `list[Optional[int]]` → items = `{"type": "integer", "nullable": true}` (2 keys). Only basic element types like `list[str]` produce single-key items. The fix is to change `Items any` to `Items *ToolProperty` (and likewise `AdditionalProperties *ToolProperty`, `PrefixItems []ToolProperty`) so that struct field ordering applies recursively at all nesting levels. This is part of the same `ToolProperty` struct rewrite — no additional code change beyond using the correct types.
...
**GLM verification note (2026-03-21):** GLM-4 templates are embedded in `tokenizer_config.json` on HuggingFace (not as separate `chat_template.jinja` files), which is why earlier searches missed them. All GLM chat models have Jinja2 templates that use `tojson` — confirmed for `THUDM/glm-4-9b-chat`, `THUDM/glm-4-9b-chat-1m`, `THUDM/GLM-4-9B-0414`, `THUDM/GLM-Z1-9B-0414`, and `zai-org/GLM-5`. Templates downloaded and verified locally. The GLM family shows a template evolution relevant to Ollama's renderers:
...
**Tool definition serialization** (the `marshalWithSpaces` code path in `model/renderers/json.go`):
...
2. **Field loss and recursive ordering** — Rewrite the `ToolProperty` struct in `api/types.go` to the exact 9-field layout specified above. Use recursive types: `Items *ToolProperty`, `AdditionalProperties *ToolProperty`, `PrefixItems []ToolProperty` (not `any` / `[]any`) so that Go's `json.Marshal` produces training-consistent key ordering at ALL nesting levels — not just at the top-level property, but inside `items` sub-objects for complex element types like `list[dict[str, int]]` and `list[Optional[int]]`. Add `Nullable *bool` at position 6. This single struct change fixes field loss, `enum`/`description` ordering, AND recursive nested key ordering simultaneously.

**Tool call argument serialization** (the `formatToolCallArgument` / `marshalQwenToolCallArgument` code path in `model/renderers/qwen3coder.go` — a separate code path with a different failure mode; tool call arguments are round-tripped through the parser and re-serialized in historical assistant messages, causing KV cache invalidation in addition to training distribution shift):
3. ~~**Boolean/None capitalization**~~ — **DONE (2026-04-03).** Added `case bool:` returning `"True"`/`"False"`, changed nil to return `"None"`, added `case float64:` with `strconv.FormatInt` for integer-valued floats. `TestFormatToolCallArgumentMatchesOfficialTemplate` 14 cases (13 PASS, 1 FAILS). `TestQwen35RendererUsesXMLToolCallingFormat` 2/2 PASS. `AssistantPrefillWithThinking/think_switch_with_tool_history` exercises all four scalar paths (string, bool, int, nil) in a rendered tool history context.
4. **`very_large_int_float` int64 overflow** (independent of the struct rewrite) — `float64(1e20)` overflows `int64` in `formatToolCallArgument`, producing `"-9223372036854775808"` instead of `"1e+20"`. Fix: add `v >= math.MinInt64 && v <= math.MaxInt64` guard before `int64` conversion in `model/renderers/qwen3coder.go`. One-line fix, no dependency on anything else, can be applied at any time. Test: `FormatToolCallArgumentMatchesOfficialTemplate/very_large_int_float` FAILS by design.
5. **Object key ordering** — The `parseValue()` function in `model/parsers/qwen3coder.go` uses `json.Unmarshal` into `map[string]any` for object argument values, destroying key ordering. The renderer then alphabetizes via `json.Encoder`. Fix requires an insertion-order-preserving JSON parser (the same approach llama.cpp uses via `nlohmann::ordered_json`). Three KV cache round-trip tests (cases 10, 11, 15) and two number normalization tests (cases 12, 13) already exist and correctly fail, documenting the bug.

For Qwen 3.5 specifically, the current state is: (1a) `TestQwen35RendererToolDefinitionsMatchOfficialTemplate` Tool 1 (`get_weather` with `list[str]`) **passes** — byte-exact against HF ground truth. (1b) Tool 2 (`choose_color` with `Literal["red", "green", "blue"]`) **FAILS by design** — enforces `enum`/`description` field ordering (added 2026-04-01). (1c) Tool 3 (`search_records` with `list[dict[str, int]]`) **FAILS by design** — enforces multi-key `items` sub-object key ordering (added 2026-04-03). (1d) Three `silent_field_loss` subtests **FAIL by design** — enforce field preservation through JSON unmarshal→render for `nullable`, `additionalProperties`, `prefixItems` (added 2026-04-03). (2) The `ToolProperty` struct rewrite in `api/types.go` (the 9-field layout specified above) fixes all 5 failing subtests simultaneously. (3) The full round-trip consistency test in `server/qwen35_kvcache_roundtrip_test.go` must pass completely — including the 5 currently-failing cases for object key ordering and number normalization.
...
### 4.4 Sub-Bug D — Scalar Boolean and None Capitalization in `formatToolCallArgument` — DONE (2026-04-03)

The official Qwen 3.5 Jinja2 template (in the tool call arguments rendering section of `chat_template.jinja`) renders tool call arguments using two code paths based on the argument's type:
...
**Fix applied (2026-04-03):** Three new `case` branches were added to `formatToolCallArgument` in `model/renderers/qwen3coder.go`, and the nil branch was updated:
- `nil`: changed from `return "null"` to `return "None"` (matching Python `str(None)`)
- `case bool:`: returns `"True"` or `"False"` (matching Python `str(True)`, `str(False)`)
- `case float64:`: integer-valued floats use `strconv.FormatInt(int64(v), 10)` instead of `fmt.Sprintf("%v", v)`, producing `"10000000000"` instead of `"1e+10"` (matching Python `str(10000000000)`). **Known remaining bug:** no range check before `int64` conversion — `float64(1e20)` overflows `int64`, producing garbage `"-9223372036854775808"` instead of `"1e+20"` (what Python `str(1e20)` and Go `fmt.Sprintf("%v", 1e20)` both produce). Fix: add `v >= math.MinInt64 && v <= math.MaxInt64` guard. Caught by `TestFormatToolCallArgumentMatchesOfficialTemplate/very_large_int_float`.

Imports `math` and `strconv` were added to `qwen3coder.go`.

**Byte-exact verification:** Both HuggingFace Transformers v5.3.0 and Go `Qwen35Renderer` outputs were saved to files and diffed for a conversation containing past tool calls with boolean and None arguments. After the fix, the diff shows **zero differences** for all scalar types:

| Argument value | HuggingFace Transformers renders | Go renders | Match? |
|---|---|---|---|
| Scalar `True` | `True` | `True` | **Yes** — fixed 2026-04-03 |
| Scalar `False` | `False` | `False` | **Yes** — fixed 2026-04-03 |
| Scalar `None` | `None` | `None` | **Yes** — fixed 2026-04-03 |
| Large integer `10000000000` | `10000000000` | `10000000000` | **Yes** — fixed 2026-04-03 |
| Very large float `1e20` | `1e+20` | `-9223372036854775808` | **No** — int64 overflow, fix: range check |
| `True` inside a list | `true` | `true` | **Yes** (both use JSON) |
| `None` inside a dict | `null` | `null` | **Yes** (both use JSON) |

This fix is scoped to the Qwen renderers. `formatToolCallArgument` has exactly two production callers: `qwen35.go:168` (model `"qwen3.5"`) and `qwen3coder.go:187` (model `"qwen3-coder"`). No other renderer calls it — the Qwen3VL renderer uses `marshalWithSpaces` on the whole arguments object at `qwen3vl.go:116` (JSON path, not per-argument XML). Both the Qwen 3.5 template (line 122 of `chat_template.jinja` from `Qwen/Qwen3.5-27B`) and the Qwen3-Coder template (line 89 of `Qwen3-Coder.jinja` from llama.cpp's template collection) have byte-identical argument rendering logic: `args_value | string` for scalars, `args_value | tojson | safe` for collections. Jinja2's `| string` calls Python `str()` — verified by running the filter in isolation (`true | string` → `True`) and by rendering both full templates with boolean arguments.

**Test status (2026-04-03):** Three tests enforce the correct behavior: (1) `TestFormatToolCallArgumentMatchesOfficialTemplate` in `qwen3coder_test.go` (unit test, 14 cases: 13 PASS, 1 FAILS — `very_large_int_float` catches int64 overflow for `float64(1e20)`, producing garbage instead of `"1e+20"`; was 7/11 before the bool/None fix), (2) `TestQwen35RendererUsesXMLToolCallingFormat` in `qwen35_test.go` (integration test, 2/2 subtests PASS — was 0/2 before the fix), (3) `TestQwen35RendererAssistantPrefillWithThinking/think_switch_with_tool_history` (integration test, exercises all four scalar paths — string, bool, int, nil — in rendered tool history). The dead `TestFormatToolCallArgumentThinkingVL` (which tested `formatToolCallArgument` from a VL test file despite VL never calling it, with wrong expected values) was deleted on 2026-04-01.
...
llama.cpp avoids all three sub-bugs by executing the Jinja2 template directly. Its `tojson` implementation (in llama.cpp's `tojson` implementation in `value.cpp`) defaults to spaced separators, preserves dict insertion order (via `as_ordered_object()` with comment "IMPORTANT: need to keep exact order"), and writes `<>&` literally.

However, llama.cpp's `tojson` has **one minor flaw**: float serialization (in llama.cpp's `tojson` float serialization in `value.cpp`) uses C++ `ostringstream` default formatting (6 significant digits), while Python's `json.dumps` preserves ~17 digits. Additionally, `1.0` becomes `1` in C++ but stays `1.0` in Python. Unlikely to affect model behavior in practice.
...
llama.cpp builds a PEG grammar from the declared tool schemas (in llama.cpp's PEG grammar builder in `chat.cpp`, at reference commit `a0ed91a`) before generation begins. The grammar is converted to GBNF and applied as logit masks at every token step. The model literally cannot produce an invalid tool call.
...
**Versions affected:** llama.cpp up to and including `c5a7788`. **Fixed in:** post-refactor versions (`0cd4f47`+, March 10, 2026), verified in local clones at commits `0cd4f47` and `aa2d278`.

**The bug (in affected versions):** The per-model specialized handler `common_chat_params_init_qwen3_coder` in `chat.cpp` called `apply(tmpl, inputs)` without including `enable_thinking` in the Jinja template context. Since `enable_thinking` was always `undefined`, the template always produced `<think>\n`. llama.cpp then post-processed: when `!inputs.enable_thinking` and the prompt ended with `<think>\n`, it appended `</think>`, producing `<think>\n</think>` (3 tokens). The official non-thinking prefill is `<think>\n\n</think>\n\n` (6 tokens) — with an empty line inside the think block and a double-newline after. The 3-token sequence was never seen during training. This is a real distribution shift.

**The fix (in post-refactor versions):** The per-model specialized handlers (`common_chat_params_init_qwen3_coder`, `common_chat_params_init_qwen3_vl`, etc.) were replaced with a unified `common_chat_template_direct_apply()` function. This unified path correctly passes `enable_thinking` to the Jinja template context in `chat.cpp`:
```cpp
...
In llama.cpp's prompt token feeding in `server-context.cpp`, llama.cpp feeds ALL prompt tokens into the penalty sampler. Currently masked by `repeat_penalty=1.0` default.

### 7.3 Bug 3 — No Model-Specific Sampler Defaults

llama.cpp uses the same hardcoded defaults for all models in llama.cpp's hardcoded sampling defaults in `common.h`. Qwen recommends `temp=0.6`, `top_p=0.95`, `top_k=20`, `min_p=0`, `repeat_penalty=1.05`. No mechanism exists to read per-model recommended sampling parameters.

### 7.4 Bug 4 — Float Precision Loss in `tojson`

Float serialization in llama.cpp's `tojson` float serialization in `value.cpp` uses 6 significant digits versus Python's ~17. Also, `1.0` renders as `1` instead of `1.0`. Very unlikely to affect model behavior.
...
The compatibility test in the speculative decoding compatibility check in `speculative.cpp` checks partial tail removal on the model's memory. For hybrid models, the recurrent sub-cache's tail removal rejection in `llama-memory-recurrent.cpp` explicitly rejects it. Both `LLM_ARCH_QWEN35` and `LLM_ARCH_QWEN3NEXT` are classified as hybrid → speculative decoding disabled.
...
The `numParallel` restriction for hybrid architectures in `server/sched.go` — both the fork and upstream Ollama force `numParallel = 1` for all hybrid/recurrent architectures (`qwen35`, `qwen35moe`, `qwen3next`, `lfm2`, `lfm2moe`, `nemotron_h`, etc.).
...
The equal-token-count batch layout check in `deltanet.go` requires all sequences in a batch to have the **same token count** (`ErrUnsupportedBatchLayout`). This is a structural requirement of chunked linear attention. Different requests have different prompt lengths and can't be batched. During generation (1 token/step), they could theoretically batch.
...
| `ToolProperty` key ordering: `description` before `enum` (should be reversed for HF-trained models) | Distribution shift on tool parameters with enumerated values. Every verified model uses HF Transformers' `tojson` with `sort_keys=False` (insertion order), producing `enum` before `description`. Stock Jinja2 uses `sort_keys=True` (alphabetical: d < e → `description` first), which agrees with Go's wrong order — but the model was trained with HF's override, not stock Jinja2. Verified empirically by running both stock Jinja2 and HF `apply_chat_template` on `choose_color(color: Literal["red", "green", "blue"])`. | **OPEN — test-enforced** (`TestQwen35RendererToolDefinitionsMatchOfficialTemplate` Tool 2 FAILS) |
| `ToolProperty` field loss: `nullable`, `additionalProperties`, `prefixItems` silently dropped during `json.Unmarshal`; recursive key ordering wrong for complex element types in `items` | Unlike the `"return"` field (which is informational metadata the model handles with or without — both are in-distribution), these fields carry **type constraint semantics**: `"nullable": true` means "accepts null," and dropping it changes the schema's meaning. A developer writing `Optional[int]` in Python gets `"nullable": true` from HF's `get_json_schema()` — Ollama silently destroys it on unmarshal with no error or warning. The user cannot avoid this. Additionally, `Items any` causes key alphabetization inside items sub-objects for complex element types like `list[dict[str, int]]` (2-key items: `type` + `additionalProperties`) and `list[Optional[int]]` (2-key items: `type` + `nullable`). Fix: rewrite `ToolProperty` struct with `Items *ToolProperty`, `AdditionalProperties *ToolProperty`, `PrefixItems []ToolProperty`, `Nullable *bool` — recursive types fix both field loss AND nested key ordering in one change. | **OPEN** |
| ~~`formatToolCallArgument` scalar boolean/None/large-int~~ | ~~Go produced `true`/`false`/`null`/`1e+10`, official templates produce `True`/`False`/`None`/`10000000000`~~ | **DONE (2026-04-03)** — `case bool:` returns `"True"`/`"False"`, nil returns `"None"`, `case float64:` integer-valued uses `strconv.FormatInt`. Unit: 11/11 PASS. Integration: 2/2 PASS. |
| `ToolProperty.Items` (and `AdditionalProperties`, `PrefixItems`) key alphabetization at nested levels | Real issue for complex element types. `list[dict[str, int]]` → items = `{"type": "object", "additionalProperties": {"type": "integer"}}` (2 keys, Go alphabetizes to `additionalProperties` before `type`). `list[Optional[int]]` → items = `{"type": "integer", "nullable": true}` (2 keys, Go puts `nullable` before `type`). Only basic types like `list[str]` are single-key. Empirically verified via the `prove_multikey_items.py` verification script. Fix: change `Items any` to `Items *ToolProperty`, `AdditionalProperties any` to `*ToolProperty`, `PrefixItems []any` to `[]ToolProperty` — struct field ordering then applies recursively. This is part of the same struct rewrite, no additional code change. Affects every model trained with HF Transformers (all of them — the `tojson` override is global, no model can opt out). | **OPEN** — merged into the P2 `ToolProperty` struct rewrite |
...
**The trap: `FROM hf.co/unsloth/...`:** Ollama auto-discovers the mmproj file, classifies it as a projector, then the Go engine rejects the entire load because `qwen3next` is in `OllamaEngineRequired()` with no fallback. The custom Modelfile uses `FROM` with the locally downloaded GGUF file (manually downloaded) to avoid this.
...
Ollama bundles vision tensors into the main GGUF (1307 tensors). llama.cpp uses a separate mmproj GGUF loaded via `--mmproj`. Unsloth follows llama.cpp's convention. Ollama's Go engine explicitly rejects split vision in the split-vision rejection logic in `llm/server.go`.
...
- **BOS/EOS logging bug in the BOS/EOS logging check in `vocabulary.go`**: Checks `v.BOS` where it should check `v.EOS`. Purely cosmetic — the actual EOS append runs unconditionally. Never fires for Qwen 3.5 (`AddEOS` defaults to `false`).
...
| ~~**P1**~~ | ~~Fix `ToolFunctionParameters` key ordering: move `Properties` before `Required` in `api/types.go`~~ | ~~Small~~ | **DONE** — struct reordered. `TestQwen35RendererToolDefinitionsMatchOfficialTemplate` Tool 1 (get_weather) verifies this passes. |
| ~~**P2**~~ | ~~Fix `ToolProperty` key ordering (`enum`/`description` swapped)~~ — merged into the struct rewrite below (the `enum`/`description` swap is part of the same 9-field struct change). | ~~Small~~ | **Merged** into struct rewrite below |
| **P2** | Fix `ToolProperty` field loss, ordering, and recursive nested key ordering — rewrite the `ToolProperty` struct in `api/types.go` to the exact 9-field layout specified in Section 4.2. Uses recursive types: `Items *ToolProperty`, `AdditionalProperties *ToolProperty`, `PrefixItems []ToolProperty`. Adds `Nullable *bool` at position 6, moves `Enum` to position 7 and `Description` to position 8. This single struct change fixes field loss, `enum`/`description` ordering, AND recursive nested key ordering simultaneously. **Test enforcement (2026-04-01):** `TestQwen35RendererToolDefinitionsMatchOfficialTemplate` Tool 2 (choose_color with `Literal["red", "green", "blue"]`) FAILS on `enum`/`description` ordering — Go produces `description` before `enum` but training data has `enum` first. Critical `tojson` clarification: stock Jinja2 `sort_keys=True` agrees with Go's wrong order (d < e), but the model was trained with HF's `sort_keys=False` override — verified empirically. | Small-Medium | **OPEN — test-enforced** |
| ~~**P2**~~ | ~~Fix `formatToolCallArgument` scalar type rendering~~ — rewrite type handling in `model/renderers/qwen3coder.go`: (1) `bool`: `true`→`"True"`, `false`→`"False"`. (2) `nil`: `"null"`→`"None"`. (3) `float64` integer-valued: `strconv.FormatInt(int64(v), 10)`. Matches Python `str()` used by both the Qwen 3.5 and Qwen3-Coder templates' `| string` filter (both templates verified identical). | ~~Small~~ | **DONE (2026-04-03)** — `TestFormatToolCallArgumentMatchesOfficialTemplate` 11/11 PASS, `TestQwen35RendererUsesXMLToolCallingFormat` 2/2 PASS |
...
| ~~P2~~ | Fix `formatToolCallArgument` scalar bool/None/large-int capitalization (Sub-bug D) | (2026-04-03) | **DONE** |
...
| `model/renderers/qwen3coder.go` | Shared qwen3-coder renderer — thinking support, prefill fix, spaced JSON `formatToolCallArgument`, Python `str()` capitalization for scalar bool/None/large-int |
