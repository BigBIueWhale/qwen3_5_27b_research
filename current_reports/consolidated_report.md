...
Note: the old test's out-of-training-distribution `items` example (`{"type": "string", "description": "..."}`) has been replaced (2026-04-01). `TestQwen35RendererToolDefinitionsMatchOfficialTemplate` now uses byte-exact HF ground truth with realistic single-key `items` (`list[str]` → `{"type": "string"}`), `description` at the property level, and both parameters in `required`. The test passes — the simple-tool serialization path produces byte-identical output to HF Transformers. The real multi-key `items` sub-objects come from complex element types via `_parse_type_hint()` recursion (e.g., `list[dict[str, int]]` → items = `{"type": "object", "additionalProperties": {"type": "integer"}}`), not from `description` — these are tested by the proposed `TestQwen35ToolPropertyRoundTripFidelity` (cases 4-5), which arrives with the `ToolProperty` struct rewrite.
...
For Qwen 3.5 specifically, the current state is: (1a) `TestQwen35RendererToolDefinitionsMatchOfficialTemplate` **passes** for the simple tool case (`get_weather` with `list[str]`) — byte-exact against HF ground truth (rewritten 2026-04-01). (1b) The same test needs a Tool 2 (enum) case added alongside the `ToolProperty` struct rewrite — this will verify `enum`/`description` field ordering with byte-exact HF ground truth. (2) The new `TestQwen35ToolPropertyRoundTripFidelity` (not yet implemented) must pass all 5 cases — `nullable` field preserved at property level, `additionalProperties` field preserved at property level, `prefixItems` field preserved at property level, `nullable` key ordering preserved inside items sub-objects for `list[Optional[int]]`, and `additionalProperties` key ordering preserved inside items sub-objects for `list[dict[str, int]]` (tests the full `json.Unmarshal` → `marshalWithSpaces` round-trip against byte-exact HuggingFace Transformers ground truth). (3) The full round-trip consistency test in `server/qwen35_kvcache_roundtrip_test.go` must pass completely — including the 5 currently-failing cases for object key ordering and number normalization.
...
| ~~**P1**~~ | ~~Fix `ToolFunctionParameters` key ordering: move `Properties` before `Required` in `api/types.go`~~ | ~~Small~~ | **DONE** — struct reordered, all tests updated. `TestQwen35RendererToolDefinitionsMatchOfficialTemplate` rewritten (2026-04-01) with byte-exact HF ground truth: realistic `items` (`list[str]` → `{"type": "string"}`), both params in `required`, `description` at property level not inside `items`. Old out-of-training-distribution `items` example removed. |
...
| **P2** | Fix `ToolProperty` field loss, ordering, and recursive nested key ordering — rewrite the `ToolProperty` struct in `api/types.go` to the exact 9-field layout specified in Section 4.2 (verified against every HuggingFace Transformers v5.3.0 type combination, plus empirically verified for complex element types via the `prove_multikey_items.py` verification script). Uses recursive types: `Items *ToolProperty`, `AdditionalProperties *ToolProperty`, `PrefixItems []ToolProperty` (not `any` / `[]any`) so that `json.Marshal` produces correct key ordering at ALL nesting levels — including inside `items` sub-objects for `list[dict[str, int]]` (items has 2 keys: `type`, `additionalProperties`) and `list[Optional[int]]` (items has 2 keys: `type`, `nullable`). Adds `Nullable *bool` at position 6, moves `Enum` to position 7 and `Description` to position 8. This single struct change fixes field loss, `enum`/`description` ordering, AND recursive nested key ordering simultaneously. The lost fields carry type constraint semantics (unlike the `"return"` field, which is informational and harmless to drop) — `"nullable": true` means "accepts null," and dropping it changes the schema's meaning. Any developer using `Optional[T]` in Python with HF's `get_json_schema()` hits this silently. Affects every model trained with HF Transformers — the `tojson` override with `sort_keys=False` is hardcoded globally at `_cached_compile_jinja_template()` in `chat_template_utils.py`, no model can opt out. | Small-Medium | **OPEN** |
