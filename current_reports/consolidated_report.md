...
Every verified model (Qwen 3.5, Qwen3VL, OLMo3, OLMo3.1, and the GLM-4 family) uses HuggingFace Transformers' insertion-order `tojson` override, which produces `enum` before `description`. No verified model uses stock Jinja2's `sort_keys=True`. The HuggingFace `tojson` override is hardcoded globally in `_cached_compile_jinja_template()` at `chat_template_utils.py:476` — there is no per-model opt-out, and no model in the HF ecosystem was trained with `sort_keys=True`. llama.cpp confirms this from the other side: its Jinja engine explicitly `throw not_implemented_exception("tojson sort_keys=true not implemented")` and uses `as_ordered_object()` with the comment `"IMPORTANT: need to keep exact order"` at `value.cpp:1330`. **Insertion order is the universal training format for every tool-calling model.**
...
For Qwen 3.5 specifically, success means: (1) the rewritten `TestQwen35RendererToolDefinitionsMatchOfficialTemplate` in `model/renderers/qwen35_test.go` passes for all tool types including enum parameters (tests rendering with Go-constructed types), (2) the new `TestQwen35ToolPropertyRoundTripFidelity` in `model/renderers/qwen35_test.go` passes all 5 cases — `nullable` field preserved at property level, `additionalProperties` field preserved at property level, `prefixItems` field preserved at property level, `nullable` key ordering preserved inside items sub-objects for `list[Optional[int]]`, and `additionalProperties` key ordering preserved inside items sub-objects for `list[dict[str, int]]` (tests the full `json.Unmarshal` → `marshalWithSpaces` round-trip against byte-exact HuggingFace Transformers ground truth), and (3) the full round-trip consistency test in `server/qwen35_kvcache_roundtrip_test.go` passes completely — including the 5 currently-failing cases for object key ordering and number normalization.
