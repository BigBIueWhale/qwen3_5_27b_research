...
3. **Boolean/None capitalization** — Add `bool` type switch cases and update `nil` handling in `formatToolCallArgument` in `model/renderers/qwen3coder.go`. Matches Python's `str()` function used by the `| string` filter in both the Qwen 3.5 and Qwen3-Coder official Jinja2 templates.
...
Go's `formatToolCallArgument` in `model/renderers/qwen3coder.go` uses `fmt.Sprintf("%v", value)` for non-string, non-collection types. `fmt.Sprintf("%v", true)` produces `true` (lowercase), not `True`. For `nil`, the function hardcodes `"null"`, not `"None"`.
...
**Test enforcement (2026-04-01):** Two tests enforce the correct behavior: (1) `TestFormatToolCallArgumentMatchesOfficialTemplate` in `qwen3coder_test.go` (unit test, 11 cases, FAILS on 4: bool_true, bool_false, nil, large_int — replaces the old `TestFormatToolCallArgument` that locked in Go's wrong output), (2) `TestQwen35RendererUsesXMLToolCallingFormat` in `qwen35_test.go` (integration test, FAILS on `true` vs `True` in full render). The dead `TestFormatToolCallArgumentThinkingVL` (which tested `formatToolCallArgument` from a VL test file despite VL never calling it, with wrong expected values) was deleted.
...
| `formatToolCallArgument` scalar boolean/None/large-int: Go produces `true`/`false`/`null`/`1e+10`, official templates produce `True`/`False`/`None`/`10000000000` | Different token IDs for past tool call arguments in multi-turn conversations. Fix: add `case bool:` returning `"True"`/`"False"`, change nil to return `"None"`, add integer-valued float64 detection with `strconv.FormatInt`. Function has exactly 2 production callers: `qwen35.go:168` and `qwen3coder.go:187` — VL uses `marshalWithSpaces` (JSON path), not this function. Both Qwen 3.5 (template line 122) and Qwen3-Coder (template line 89) templates verified to use byte-identical `args_value \| string` logic for scalars. | **OPEN — test-enforced** (unit: 4 of 11 cases FAIL; integration: bool FAILS in full render) |
...
| **P2** | Fix `formatToolCallArgument` scalar type rendering — rewrite type handling in `model/renderers/qwen3coder.go`: (1) `bool`: `true`→`"True"`, `false`→`"False"`. (2) `nil`: `"null"`→`"None"`. (3) `float64` integer-valued: use `strconv.FormatInt(int64(v), 10)` to produce `"10000000000"` instead of `"1e+10"`. Matches Python `str()` used by both the Qwen 3.5 and Qwen3-Coder templates' `| string` filter (both templates verified identical). **Test enforcement (2026-04-01):** Two tests enforce this: (1) `TestFormatToolCallArgumentMatchesOfficialTemplate` (unit, 11 cases, FAILS on 4: bool_true, bool_false, nil, large_int) replaces the old `TestFormatToolCallArgument` that locked in Go's wrong output; (2) `TestQwen35RendererUsesXMLToolCallingFormat` (integration, FAILS on `true` vs `True` in full render). Both give the exact fix. Ground truth verified against both Qwen 3.5 (template line 122) and Qwen3-Coder (template line 89) — byte-identical `args_value | string` logic. `TestFormatToolCallArgumentThinkingVL` deleted — it tested `formatToolCallArgument` which the VL renderer doesn't call (VL uses `marshalWithSpaces` on the whole arguments object), and had wrong expected values that would block the fix. | Small | **OPEN — test-enforced** |
