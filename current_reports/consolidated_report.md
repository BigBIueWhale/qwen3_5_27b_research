...
| `ToolProperty` field loss: `nullable`, `additionalProperties`, `prefixItems` silently dropped during `json.Unmarshal` | Unlike the `"return"` field (which is informational metadata the model handles with or without — both are in-distribution), these fields carry **type constraint semantics**: `"nullable": true` means "accepts null," and dropping it changes the schema's meaning. A developer writing `Optional[int]` in Python gets `"nullable": true` from HF's `get_json_schema()` — Ollama silently destroys it on unmarshal with no error or warning. The user cannot avoid this. `Optional[T]` parameters are extremely common. Fix: add `Nullable *bool`, `AdditionalProperties any`, `PrefixItems []any` fields to `ToolProperty` in `api/types.go`. | **OPEN** |
