...
As of 2026-03-10, one of the 3 JSON serialization bugs is fixed in the fork, while upstream Ollama still has all three. Verified against the official template at `/tmp/qwen35_template.jinja2` (line 50 for tool definitions via `tojson`, line 122 for tool call arguments via `tojson`):

- **HTML escaping:** This is now fixed in the fork as of 2026-03-11. `marshalWithSpaces` no longer inherits `json.Marshal`'s HTML escaping of `<>&`, and the fix was applied across the full tool-definition serialization graph, not just the top-level helper. `marshalWithSpaces` is shared infrastructure: called by the Olmo3, Qwen 3.5, and Qwen3VL renderers. The official HuggingFace templates for `Qwen/Qwen3.5-*`, `Qwen/Qwen3-VL-*`, `allenai/Olmo-3-7B-Instruct`, and `allenai/Olmo-3.1-32B-Instruct` all use `tools | tojson` for tool definitions, so literal `<>&` are the expected bytes. The final patch covered both `json.go` and the nested custom `MarshalJSON` implementations in `api/types.go` / `internal/orderedmap`. `formatToolCallArgument` in `qwen3coder.go` was already fixed earlier for the Qwen-local tool-call-argument path.
...
- **Compact separators in tool call arguments:** This was true as of 2026-03-08, but is now fixed in the fork as of 2026-03-10. `formatToolCallArgument` no longer uses raw `json.Marshal`; it now emits spaced JSON with HTML escaping disabled for the Qwen-family tool call argument path. This leaves shared `marshalWithSpaces` callers and key ordering as the remaining open serialization gaps.
...
Sub-bug C is fixed in the fork's Qwen-local serializer path, and Sub-bug A is now also fixed in the shared tool-definition path. Key ordering (corruption 2) still requires either preserving raw JSON strings through the parse→store→render cycle, or using ordered maps / renderer-local ordering control for nested objects.
...
|----------|------|--------|----------------------|
| ~~**P0 CRITICAL**~~ **DONE** | Fix compact separators in `formatToolCallArgument` (Sub-bug C) — implemented 2026-03-10 in the fork's Qwen-local serializer path. `object`/`array` tool call arguments now render with spaced JSON, and HTML escaping is disabled in that same path. | Small | 3.1 Bug C |
| ~~**P0 CRITICAL**~~ **DONE** | Fix shared HTML escaping in the full tool-definition serialization path (Sub-bug A) — completed 2026-03-11. The final patch covered `marshalWithSpaces` and the nested tool-type `MarshalJSON` paths in `api/types.go` / `internal/orderedmap`, so `<>&` now remain literal in shared tool-definition serialization. | Medium | 3.1 Bug A |
| **P1** | Fix `required`/`properties` key ordering (Sub-bug B) — **DO NOT change the shared struct without per-model template verification** — requires either verifying every model's HuggingFace template or implementing renderer-local marshaling. For Qwen 3.5 specifically, success should mean the full round-trip consistency test in [`server/qwen35_kvcache_roundtrip_test.go`](/home/user/shared_vm/ollama/server/qwen35_kvcache_roundtrip_test.go) passes completely. | Medium-High (prerequisite research across all tool-calling models) | 3.1 Bug B |
