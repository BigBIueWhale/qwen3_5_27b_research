...
- **HTML escaping:** `marshalWithSpaces` at `json.go:9` correctly adds spacing after `:` and `,` but inherits `json.Marshal`'s HTML escaping of `<>&` — the spacing is right, the escaping is wrong. `marshalWithSpaces` is shared infrastructure: called by the Olmo3, Qwen 3.5, and Qwen3VL renderers. This shared fix is still open in the fork, but it has now been verified as **correct in principle** for all current callers: the official HuggingFace templates for `Qwen/Qwen3.5-*`, `Qwen/Qwen3-VL-*`, `allenai/Olmo-3-7B-Instruct`, and `allenai/Olmo-3.1-32B-Instruct` all use `tools | tojson` for tool definitions, so literal `<>&` are the expected bytes. **Implementation correction from failed testing on 2026-03-11:** this is not a `marshalWithSpaces`-only patch. Nested tool-definition fields still pass through custom `MarshalJSON` implementations in `api/types.go`, so the real fix must cover the full tool-definition serialization path for escaping. Additionally, `formatToolCallArgument` in `qwen3coder.go` called raw `json.Marshal` directly (a separate code path), and that Qwen-local path is already fixed.
...
Sub-bug C is now fixed in the fork's Qwen-local serializer path, and the Qwen-local HTML-escaping problem for tool call arguments is fixed with it. Key ordering (corruption 2) still requires either preserving raw JSON strings through the parse→store→render cycle, or using ordered maps for nested objects. Shared `marshalWithSpaces` HTML escaping is also still open for tool definitions.
...
| **P0 CRITICAL** | Fix remaining shared HTML escaping in the full tool-definition serialization path (Sub-bug A) — the Qwen-local `formatToolCallArgument` path is now fixed, and the current `marshalWithSpaces` callers (`qwen3.5`, `qwen3-vl`, `olmo3`) are verified correct targets because their official templates use `tools | tojson` for tool definitions. The fix remains **unimplemented** in the fork as of 2026-03-11. Failed implementation testing showed that helper-only changes are insufficient because nested tool-definition fields still escape through custom `MarshalJSON` paths in `api/types.go`. Empirically verified: `<>&` → `\u003c\u003e\u0026` remains open for shared tool-definition serialization. | Medium (shared function plus nested tool-type serialization boundaries; caller verification for current users complete) | 3.1 Bug A |
