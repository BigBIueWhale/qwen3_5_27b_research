...
| **P0 CRITICAL** | Fix remaining shared HTML escaping in `marshalWithSpaces` (Sub-bug A) — the Qwen-local `formatToolCallArgument` path is now fixed, and the shared helper has now been verified as correct to change for all its current runtime callers (`qwen3.5`, `qwen3-vl`, `olmo3`) because their official templates use `tools | tojson` for tool definitions. The fix remains **unimplemented** in the fork as of 2026-03-11. Empirically verified: `<>&` → `\u003c\u003e\u0026` remains open for shared tool-definition serialization. | Small-Medium (shared function, caller verification for current users now complete) | 3.1 Bug A |
