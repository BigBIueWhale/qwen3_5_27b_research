...
| **P0 CRITICAL** | Fix remaining shared HTML escaping in `marshalWithSpaces` (Sub-bug A) — the Qwen-local `formatToolCallArgument` path is now fixed, but shared serializer callers still need verification before a global change. Empirically verified: `<>&` → `\u003c\u003e\u0026` remains open for shared tool-definition serialization. | Small-Medium (shared function, must verify Olmo3/Qwen3VL callers) | 3.1 Bug A |
