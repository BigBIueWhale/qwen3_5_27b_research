...
**Failure mode 1 — training distribution shift.** The model's learned attention patterns were optimized for the exact token sequence the official Jinja2 template produces. A missing space, a swapped JSON key, or an HTML-escaped angle bracket changes the token IDs the model sees, pushing the input out of the training distribution. The model still runs and produces output, but its tool-calling accuracy, reasoning quality, and instruction-following reliability all degrade. The degradation is silent — the model doesn't error, it just gets worse. Users blame the model ("Qwen 3.5 is unreliable") rather than the inference engine. This failure mode applies to every part of the rendered prompt — tool definitions in the system prompt, historical assistant messages, tool results, and the generation prompt.

**Failure mode 2 — KV cache invalidation in multi-turn agentic conversations.** Ollama re-renders the entire conversation history on every turn. In a 20-turn agentic coding session with tool calls, the engine checks whether the new prompt shares a byte-identical prefix with the previous prompt. If it does, the KV cache (the expensive intermediate computation from processing all prior tokens) is reused. If any byte differs — a space removed, a key reordered, an `&` escaped to `\u0026` — the cache is invalidated from that point forward, and everything after must be recomputed. By turn 15-20, each response takes dramatically longer. The model still produces correct output, so the problem is invisible in functional testing. This failure mode primarily affects **historical tool call arguments** — the model generates a tool call with arguments in a specific byte sequence, Ollama's parser extracts them into Go types, and the renderer re-serializes them on the next turn. If the re-serialized bytes differ from what the model originally generated, the KV cache is invalidated from that point. Tool definitions in the system prompt are re-serialized from the same Go structs on every turn (same code, same input, same output), so they do not cause within-conversation KV cache drift — their bugs manifest only as failure mode 1 (training distribution shift).
...
**Current coverage:** All ten renderer tests have both think=true and think=false subtests. Two tests have explicit cross-mode assertions that directly compare renderer output between think modes: `ToolDefinitionsMatchOfficialTemplate` asserts system prompt byte-identity, and `BackToBackToolCallsAndResponses` asserts full-history byte-identity (everything before the generation prompt) and uses a shared `wantHistory` constant so subtests compose `want = wantHistory + suffix`. `InterleavedThinkingAndTools/think=false` is the primary enforcer for the `<think>` wrapping gate — its post-lastQueryIndex assistants are the only place where re-adding `isThinking &&` to the wrapping condition would change the output (`BackToBackToolCallsAndResponses`' assistant is before lastQueryIndex, so `i > lastQueryIndex` is already false regardless of `isThinking`). `StructuredToolArgumentsUseSpacedJSON/think=false` covers post-lastQueryIndex with empty reasoning. `AssistantToolCallIsNotPrefill/think=false` covers the prefill guard. `AssistantPrefillWithThinking` subtests 1/2 and 3/4 share the SAME want constants across think modes — compile-time proof of byte-identity for the prefill path (Gap 12 — DONE). Subtest 5 covers a full agentic loop under think=false with reasoning preserved. `NoThinkPrefill` covers both generation prompt variants. **Complete coverage — no remaining P1 gaps at the renderer level.**

**Related gaps:** Gap 2 (DONE), Gap 5 (OPEN — Property 1 at the KV cache level), Gap 12 (DONE).
...
**Current coverage:** `BackToBackToolCallsAndResponses` has a pre-lastQueryIndex assistant with targeted diagnostic. `InterleavedThinkingAndTools` has two post-lastQueryIndex assistants. `AssistantPrefillWithThinking` subtest 5 has both sides (pre: reasoning discarded, post: reasoning preserved with `<think>` wrapping). Subtest 6 has both sides in one conversation with byte-exact verification. **Complete coverage — both sides of the boundary verified byte-exact (Gap 12, Gap 13 — DONE).**

**Related gaps:** Gap 12 (DONE), Gap 13 (DONE — covered by Gap 12 subtests 5 and 6).
...
**Current coverage:** `AssistantPrefillWithThinking` — 6 subtests covering all meaningful prefill scenarios: with reasoning (think=true/false), without reasoning (think=true/false), full agentic loop with tool history (think=false), both sides of lastQueryIndex boundary. Every subtest uses `prefillDiagnostics` to verify no trailing `<|im_end|>` and no generation prompt. `AssistantToolCallIsNotPrefill` — negative test (tool calls prevent prefill). **Complete coverage (Gap 12 — DONE).**
...
**Current coverage:** `SplitQwen35ReasoningContent` tests this at the function level (19 subtests including path equivalence assertion, all code path combinations covered — nested `<think>`, close-only + multi-`</think>`, empty remaining, open-only, empty-reasoning-via-Path-2, plus whitespace edge cases enforcing `TrimLeft("\n")` not `TrimSpace` on remaining content). `AssistantPrefillWithThinking/path_equivalence` tests this at the **renderer integration level** — three sub-subtests feed three differently-encoded messages through `Render()` and assert all produce byte-identical output against the same `wantWithReasoning` constant (verified against the official template: 121 bytes with `add_generation_prompt=False`, 110 bytes after prefill stripping of `<|im_end|>\n`). **Complete coverage — unit + integration (DONE).**

**Whitespace, newline, and tab handling note:** The extraction function and the renderer both handle whitespace at two distinct levels, with different rules at each. (1) **Remaining content after `</think>` extraction** uses `strings.TrimLeft(content, "\n")`, matching the template's `.lstrip('\n')` — this strips **only newline characters** (`\n`), preserving spaces and tabs. Tests 15 and 16 enforce this distinction: Test 15 verifies a leading space is preserved after `</think>`, Test 16 verifies a newline is stripped but the space after it survives. If someone changed `TrimLeft("\n")` to `TrimSpace` (which strips spaces, tabs, newlines, and all Unicode whitespace), both tests would fail. (2) **Top-level message content** uses `strings.TrimSpace` at `qwen35.go:139`, matching the template's `render_content(...)|trim` at line 82 — both strip all Unicode whitespace (spaces, tabs, newlines, carriage returns) from both ends. This is implicitly tested by every byte-exact renderer test (59 subtests total), since any change to the trimming behavior would shift content bytes and break the byte-exact comparison against template ground truth.
...
**Current coverage:** `FormatToolCallArgumentMatchesOfficialTemplate` (14 cases: 13 PASS, 1 FAILS — `very_large_int_float` exposes int64 overflow for `float64(1e20)`, producing garbage instead of `"1e+20"`). `UsesXMLToolCallingFormat` (boolean `True`). `StructuredToolArgumentsUseSpacedJSON` (dict with HTML chars). `AssistantPrefillWithThinking/think_switch_with_tool_history` (string + bool + int + nil in rendered tool history). The `very_large_int_float` failure is a real code bug: `strconv.FormatInt(int64(1e20))` overflows — the fix is adding a range check before `int64` conversion. **Coverage: DONE (Gap 10). Code fix for overflow: OPEN.**
...
**Current coverage:** `ToolDefinitionsMatchOfficialTemplate` — Tool 1 PASSES, Tool 2 FAILS by design (enum/description ordering), Tool 3 FAILS by design (multi-key `items` sub-object key alphabetization — `Items any` → `map[string]any`). Three `silent_field_loss` subtests FAIL by design: `nullable` (`Optional[str]`), `additionalProperties` (`dict[str, int]`), `prefixItems` (`tuple[int, int]`) — all silently dropped during `json.Unmarshal` because `ToolProperty` struct lacks these fields. Each field loss subtest accepts either correct rendering or an error; fails only on silent corruption. **Partially done — Gap 4. All known divergences now have test enforcement.**
...
| `NoThinkPrefill` | — | — | — | — | — | — | — | ✓/✓ |
| `BackToBackToolCallsAndResponses` | **✓/✓** + history invariant | pre-LQI | — | — | int ✓ | two tools ✓ | grouped ✓ | ✓/✓ |
| `StructuredToolArgumentsUseSpacedJSON` | ✓/✓ | post-LQI (empty) | — | — | dict ✓ | — | — | ✓/✓ |
| `ToolDefinitionsMatchOfficialTemplate` | **✓/✓** + sysprompt invariant | — | — | — | — | **FAILS** T2,T3 ✓/✓; **FAILS** 3 field loss | — | — |
| `InterleavedThinkingAndTools` | **✓/✓** | post-LQI | — | — | str ✓ | two tools ✓ | non-consec ✓ | ✓/✓ |
| `AssistantPrefillWithThinking` | **✓/✓** | pre+post-LQI | **✓/✓** (6 subtests) | integration ✓ (3 encodings) | str+bool+int+nil ✓ | — | single ✓ | — |
| `AssistantToolCallIsNotPrefill` | ✓/✓ | post-LQI | neg ✓/✓ | — | str ✓ | — | — | ✓/✓ |
| `SplitQwen35ReasoningContent` | — | — | — | unit ✓ (18 cases) | — | — | — | — |
| `FormatToolCallArgumentMatchesOfficialTemplate` | — | — | — | — | 14 cases (13✓ 1**F**) | — | — | — |
| KV cache round-trip (22 cases) | — | — | — | — | ✓ | — | ✓ | — |

**Key:** LQI = lastQueryIndex. "pre-LQI" = assistant before boundary (no `<think>`). "post-LQI" = after boundary (`<think>` wrapping). "pre+post-LQI" = both sides verified byte-exact in one conversation. "neg" = negative test (verifies something does NOT happen).

**Holes that must be filled:**
- ~~**P1 × Prefill:** `AssistantPrefillWithThinking` has no think=false subtest → ✓/— instead of ✓/✓~~ **DONE** (Gap 12, commit `232e3b97`)
- ~~**P1 × ToolDef:** `ToolDefinitionsMatchOfficialTemplate` has no think=false subtest → —/— in P1 column~~ **DONE** (commit `664bcdf7`, system prompt byte-identity assertion added)
- ~~**P2 boundary:** No test has BOTH pre-LQI and post-LQI assistants verified byte-exact in the same conversation~~ **DONE** (Gap 12 subtests 5 and 6)
- ~~**P3 coverage:** Only 1 of 6 prefill scenarios tested; no think=false, no tool history, no empty reasoning~~ **DONE** (Gap 12, 6 subtests)
- ~~**P4 integration:** Only unit-level — renderer-level path equivalence needed to catch post-extraction bugs~~ **DONE** — `AssistantPrefillWithThinking/path_equivalence` subtests: 3 encodings (explicit field, inline both tags, inline close-only) all assert against the same `wantWithReasoning` constant verified against the official template (121 bytes → 110 after prefill strip)
- ~~**P5 × prefill subtest 5:** Tool call in `think_switch_with_tool_history` only uses string argument `"Paris"`~~ **DONE** — now exercises bool (`True`), int (`42`), nil (`None`) alongside string (`"Paris"`), enforcing all `formatToolCallArgument` scalar paths in the prefill/tool-history context
- ~~**P4 × SplitQwen35ReasoningContent:** Nested `<think>` tags and 4 other uncovered code paths~~ **DONE** (5 new subtests: nested `<think>` tags, close-only + multiple `</think>`, empty remaining from extraction, open-only without `</think>`, and empty reasoning via Path 2)
...
### ~~NBU-1: Pre-lastQueryIndex reasoning discarding~~ RESOLVED

**Resolved by Gap 12 subtests 5 and 6 (commit `232e3b97`).** Subtest 5 (`think_switch_with_tool_history`) verifies byte-exact that the first assistant's reasoning `"I should use the weather tool."` is discarded when before lastQueryIndex. Subtest 6 (`pre_lastQueryIndex_reasoning_discarded`) verifies both sides of the boundary in a single conversation. Both use byte-exact `got != want` comparison — not `strings.Contains` negative checks.

### ~~NBU-2: Prefill think-independence~~ RESOLVED

**Resolved by Gap 12 subtests 2 and 4 (commit `232e3b97`).** Subtests 1/2 share the same `wantWithReasoning` constant. Subtests 3/4 share the same `wantNoReasoning` constant. Think=false must produce byte-identical output.

### ~~NBU-3: Mixed lastQueryIndex boundary in one conversation~~ RESOLVED

**Resolved by Gap 12 subtest 6 (commit `232e3b97`).** First assistant (index 1, before lastQueryIndex=2) renders as plain content. Second assistant (index 3, after lastQueryIndex=2) renders with empty `<think>` wrapping. Both sides byte-exact in one `Render()` call.

### ~~NBU-4: Prefill after full agentic loop~~ RESOLVED

**Resolved by Gap 12 subtest 5 (commit `232e3b97`).** Full conversation: user → assistant(thinking+toolcall) → tool_result → user → assistant(prefill). Last assistant is a prefill under think=false with reasoning preserved.

### NBU-5: System prompt think-independence for tool definitions

**Resolved by commit `664bcdf7`.** `TestQwen35RendererToolDefinitionsMatchOfficialTemplate` now renders with both think=true and think=false and asserts the system prompt (from `<|im_start|>system` to `<|im_end|>`) is byte-identical. Verified by running the official template — all three `enable_thinking` values produce a byte-identical 1566-byte system prompt.
...
**`TestQwen35RendererNoThinkPrefill`**: Byte-exact `got != want` comparison of the simplest possible prompt shape: one user message, no tools, no system message, no assistant history. Has `think=true` and `think=false` subtests. The think=true output (52 bytes) ends with `<|im_start|>assistant\n<think>\n` — the open thinking block for the model to fill. The think=false output (74 bytes) ends with `<|im_start|>assistant\n<think>\n\n</think>\n\n` — the pre-closed empty block. Both verified against the official template's add_generation_prompt block. This tests the generation prompt suffix for both modes but does NOT test the prefill bug fix because the last message is a user message, not an assistant message.

**`TestQwen35RendererBackToBackToolCallsAndResponses`**: Byte-exact comparison of a multi-turn agentic conversation with two tools (`add` and `multiply`), back-to-back tool calls, grouped tool responses, and a follow-up user query. Ground truth derived from the official Qwen/Qwen3.5-27B Jinja2 chat template. Has `think=true` and `think=false` subtests. Both modes are rendered at the function level. Before the subtests, a cross-mode P1 assertion verifies the full historical portion (everything before the generation prompt) is byte-identical between think modes — the same pattern used by `ToolDefinitionsMatchOfficialTemplate` for the system prompt, extended to the entire conversation history. Both subtests compose `want = wantHistory + suffix` from a shared `wantHistory` constant, making P1 a structural guarantee. A shared `assertThinkingAbsent` helper verifies the pre-lastQueryIndex thinking omission in both modes. **Scope of the P1 assertion:** catches think-mode leaking into system prompt, user messages, tool responses, or pre-lastQueryIndex assistant rendering. Does NOT catch re-adding `isThinking &&` to the `<think>` wrapping condition, because the assistant is before lastQueryIndex (`i > lastQueryIndex` is already false) — that specific regression is `InterleavedThinkingAndTools/think=false`'s job (see Gap 2). The byte-exact comparison locks in: tool definition JSON for two tools (field ordering, spaced separators), `\n\n` separator between content and first tool call (template line 112), `\n` separator between subsequent tool calls (template line 117), tool response grouping under a single `<|im_start|>user` block, `<|im_end|>` closure, and the generation prompt suffix. Tool call arguments are integers (2, 3, 4, 5) — `formatToolCallArgument` produces identical output for both Go `int` and JSON-deserialized `float64` values for small whole numbers.

**`TestQwen35RendererStructuredToolArgumentsUseSpacedJSON`**: Verifies that when a tool call argument is a Go `map[string]any` containing `{"content": "if (x < 5 && y > 3) {}"}`, the rendered output uses spaced JSON separators (`: ` after colons, `, ` after commas) and preserves literal HTML characters (`<`, `>`, `&`) without escaping them to `\u003c`, `\u003e`, `\u0026`. Has `think=true` and `think=false` subtests with byte-exact `got != want` comparison of the entire rendered output, plus a targeted `strings.Contains` diagnostic for the spaced JSON argument substring.

**`TestQwen35RendererToolDefinitionsMatchOfficialTemplate`**: Byte-exact comparison of tool definition JSON lines against verified HuggingFace Transformers ground truth (v5.3.0 `get_json_schema()` rendered through the official `Qwen/Qwen3.5-27B` tokenizer's `apply_chat_template`). Has `think=true` and `think=false` subtests (commit `664bcdf7`). Before the subtests, a cross-mode assertion verifies the system prompt (from `<|im_start|>system` to `<|im_end|>`) is byte-identical between think modes — enforcing Property 1 for the system prompt. Verified by running the official template with `enable_thinking=True`, `False`, and `undefined` — all three produce a byte-identical 1566-byte system prompt, because the template's system prompt block (lines 45-60) has zero references to `enable_thinking`. Two tools: Tool 1 (`get_weather`, `location: str, filters: list[str]`) exercises no HTML escaping, `properties` before `required`, single-key `items`, `items` before `description` — this tool **passes** in both subtests. Tool 2 (`choose_color`, `color: Literal["red", "green", "blue"]`) exercises `enum`/`description` field ordering — this tool **FAILS by design** in both subtests because Go's `ToolProperty` struct declares `Description` before `Enum`, but HF's `get_json_schema()` inserts `enum` first and HF's `tojson` override preserves insertion order (`sort_keys=False`, not stock Jinja2's `sort_keys=True`). The error message gives the exact 9-field `ToolProperty` struct layout to apply. A targeted `strings.Contains` loop checks all three HTML escape sequences independently. Remaining open divergences not yet tested: field loss (`nullable`, `additionalProperties`, `prefixItems`) and multi-key `items` ordering — those require the same `ToolProperty` struct rewrite (see Gap 4).
...
**`TestQwen35RendererAssistantPrefillWithThinking`**: The prefill property enforcer (commit `232e3b97`, Gap 12). 6 subtests enforcing Properties 1, 2, 3, and 5 simultaneously. Every `want` string verified against the official template with `add_generation_prompt=False`, trailing `<|im_end|>\n` stripped. Subtests: (1) `think=true` — baseline with reasoning, (2) `think=false_preserves_reasoning` — same messages as #1, same `wantWithReasoning` constant (Property 1), (3) `no_reasoning_think=true` — empty `<think>` block for post-lastQueryIndex (Property 2), (4) `no_reasoning_think=false` — same messages as #3, same `wantNoReasoning` constant (Property 1), (5) `think_switch_with_tool_history` — full agentic loop with pre-lastQueryIndex reasoning discarded, post-lastQueryIndex preserved, tool response wrapping, all four scalar argument types: string `"Paris"`, bool `True`, int `42`, nil `None` (Properties 1, 2, 3, 5, 7), (6) `pre_lastQueryIndex_reasoning_discarded` — both sides of lastQueryIndex boundary byte-exact in one conversation (Property 2, 3). Targeted diagnostics before byte-exact comparison: no trailing `<|im_end|>`, no generation prompt, pre-lastQueryIndex no `<think>`, post-lastQueryIndex reasoning preserved. Regression verified: re-adding `isThinking &&` to wrapping condition causes subtests 2, 4, 5 to fail; re-adding `isThinking` parameter to `splitQwen35ReasoningContent` causes subtests 2, 5 to fail.
...
**`TestSplitQwen35ReasoningContent`**: Tests the reasoning extraction function `splitQwen35ReasoningContent` in isolation with 18 table-driven subtests plus a cross-encoding equivalence assertion (19 subtests total). Covers three client encoding paths (Path 1 explicit field, Path 2a both tags, Path 2b close-only), multi-line reasoning, close-tag-only, explicit field priority, whitespace-only thinking field, empty content, both-empty, multiple `</think>` tags, nested `<think>` tags (verifies `LastIndex` finds innermost), close-only with multiple `</think>` (else branch + `lastClose != firstClose`), empty remaining from extraction, open-tag-only without `</think>` (Path 3 despite `<think>` in content), `</think>` at content start (empty reasoning via Path 2 not Path 3), space after `</think>` preserved in remaining (enforces `TrimLeft("\n")` not `TrimSpace`), and newline-then-space after `</think>` (newline stripped, space preserved). All expected values verified against the official Qwen 3.5 Jinja2 template. Mutation testing confirms all 5 code mutations are caught: `LastIndex`→`Index` for `<think>` (Test 10), `LastIndex`→`Index` for `</think>` remaining (Tests 10, 11), entry condition checking `<think>` instead of `</think>` (Tests 10-14), removing the else branch (Test 11), and `TrimLeft("\n")`→`TrimSpace` for remaining content (Tests 15, 16). See Gap 3 for full details.

**Think mode coverage across renderer tests:** All renderer tests construct the `Qwen35Renderer` with `isThinking: true`. **All ten** renderer tests exercise both `think: true` and `think: false`: `TestQwen35RendererNoThinkPrefill` (generation prompt, both modes), `UsesXMLToolCallingFormat/think=false` (pre-lastQueryIndex, string + boolean arguments), `BackToBackToolCallsAndResponses/think=false` (pre-lastQueryIndex with Thinking field, multi-tool), `StructuredToolArgumentsUseSpacedJSON/think=false` (post-lastQueryIndex with empty reasoning), `AssistantToolCallIsNotPrefill/think=false` (prefill guard), `InterleavedThinkingAndTools/think=false` (post-lastQueryIndex, thinking preserved — Gap 2 regression detector), `AssistantPrefillWithThinking/think=false_preserves_reasoning` (prefill with reasoning — Gap 12 Property 1), `AssistantPrefillWithThinking/no_reasoning_think=false` (prefill without reasoning — Gap 12 Property 1), and `AssistantPrefillWithThinking/think_switch_with_tool_history` (full agentic loop with tool history under think=false — Gap 12 Properties 1, 2, 3). Additionally, `ToolDefinitionsMatchOfficialTemplate` renders with both think modes and asserts the system prompt is byte-identical — enforcing Property 1 for tool definitions. Every property-enforcing test now has think subtests. The coverage matrix shows ✓/✓ across all P1 cells.
...
### Shared parser tests: `model/parsers/qwen3coder_test.go` (1379 lines)
...
**Status: DONE.** Implemented as `TestQwen35RendererAssistantToolCallIsNotPrefill` in `model/renderers/qwen35_test.go` with two subtests (`think=true` and `think=false`). Both subtests use the **same messages** (assistant with Thinking field, Content, and ToolCalls) and build their `want` strings from a shared `wantHistory` constant (293 bytes, verified against the official template). Each subtest has a targeted `<|im_end|>` presence diagnostic (via shared `assertNotPrefill` helper) followed by byte-exact full output comparison. The shared constant enforces P1 (think-independence): the historical portion must be byte-identical, with only the generation prompt suffix differing.

**Think mode and P1 enforcement:** Both subtests use the same messages — the assistant has a Thinking field populated from a prior think=true turn. When think=false is passed, the historical reasoning must still appear in the output (the template never checks `enable_thinking` for history). The shared `wantHistory` constant makes this a compile-time guarantee: if someone re-introduces an `isThinking` gate to `splitQwen35ReasoningContent` or the `<think>` wrapping condition, the think=false subtest fails while think=true still passes. The "no Thinking field + tool calls" scenario is separately covered by `StructuredToolArgumentsUseSpacedJSON` under both think modes.
...
**How existing tests catch a regression:** All renderer tests have both think=true and think=false subtests. `InterleavedThinkingAndTools/think=false` is the primary regression detector — it renders post-lastQueryIndex assistants with non-empty Thinking fields under think=false, with targeted diagnostics checking that both assistants retain their `<think>` blocks. `AssistantPrefillWithThinking` subtests 1/2 and 3/4 share the same want constants across think modes, enforcing byte-identity. `AssistantToolCallIsNotPrefill` shares a `wantHistory` constant across think modes — both subtests use the same messages (with Thinking field and tool calls), so the historical portion must be identical. Re-adding an `isThinking &&` gate to the wrapping condition causes the think=false subtests to fail while think=true passes.
...
**Status: DONE.** Implemented as a `think=false` subtest on the existing `TestQwen35RendererInterleavedThinkingAndTools` test (commit `af78cfb2`). **Upgraded to byte-exact (2026-04-02):** both think=true and think=false subtests now use full `got != want` comparison against official Jinja2 template ground truth (1981 bytes for think=true, 1992 bytes for think=false — verified byte-identical between Python template output and Go renderer output via diff). The think=false subtest has two targeted `t.Errorf` diagnostics that verify both historical assistant messages retain their `<think>` blocks with full reasoning text despite think=false (the core regression detectors), followed by the byte-exact `got != want` comparison as the final `t.Fatalf` gate. The byte-exact comparison additionally locks in the system prompt, tool definition JSON, tool response wrapping, all `<|im_end|>` closures, and the generation prompt suffix — none of which were checked before the upgrade. Both regression vectors were verified: temporarily re-adding `isThinking &&` to the `<think>` wrapping condition in `Render()` causes the think=false subtest to fail (tags vanish) while think=true passes; temporarily re-adding `isThinking` to `splitQwen35ReasoningContent`'s signature causes the think=false subtest to fail (reasoning text vanishes inside empty tags) while think=true passes.
...
| `.rstrip('\n')` — strip trailing newlines (`\n` only, not spaces/tabs) from the text before `</think>` | No explicit equivalent; the final `strings.TrimSpace(reasoning)` strips all trailing whitespace (spaces, tabs, newlines, all Unicode whitespace) | `TrimSpace` is a superset of `rstrip('\n')`, but since the final `\|trim` (below) also strips all whitespace, the end result is identical — both produce the same reasoning bytes |
...
| `.lstrip('\n')` — strip leading newlines (`\n` only, not spaces/tabs) from the extracted reasoning | No explicit equivalent; the final `strings.TrimSpace(reasoning)` strips all leading whitespace (spaces, tabs, newlines, all Unicode whitespace) | Same reasoning as above — end result identical after final trim |
| `content.split('</think>')[-1].lstrip('\n')` — remaining content is everything after the **last** `</think>`, with leading **newlines only** stripped (spaces and tabs preserved) | `strings.TrimLeft(content[lastClose+len("</think>"):], "\n")` where `lastClose = strings.LastIndex(content, "</think>")` — strips leading `\n` characters only, preserving spaces, tabs, and all other characters | The template uses `split('</think>')[-1]` which takes the piece after the **last** `</think>`. The fork uses `strings.LastIndex` to find the same position. **Critical whitespace distinction:** both `.lstrip('\n')` and `TrimLeft("\n")` strip only newline characters — if the content starts with `\n content` (newline then space), the result is ` content` (space preserved). This is NOT the same as `TrimSpace` or `strip()` which would also remove the space. Tests 15 and 16 enforce this boundary: Test 15 catches `TrimSpace` being used instead of `TrimLeft("\n")` (would incorrectly strip a leading space); Test 16 catches `TrimLeft("\n ")` (would incorrectly strip a space after a newline). Also critical when content has multiple `</think>` tags: reasoning comes from before the first tag, remaining content from after the last — preventing literal `</think>` in visible content. |
| `reasoning_content\|trim` — Jinja2's `trim` filter strips all whitespace (spaces, tabs, newlines, carriage returns) from both ends | `strings.TrimSpace(reasoning)` — Go's `TrimSpace` strips all Unicode whitespace (spaces, tabs, newlines, carriage returns, and additional Unicode whitespace like `\u00a0`) from both ends | Equivalent for all realistic inputs. `TrimSpace` is technically a superset (handles Unicode whitespace that Python's `strip()` also handles via Unicode categories), but no model output contains exotic Unicode whitespace around reasoning content. |
...
**Test 9 — Multiple `</think>` tags.** `splitQwen35ReasoningContent("<think>\nfirst\n</think>\nmiddle\n</think>\nend", "")` → reasoning `"first"`, remaining `"end"`. The official template uses two different indices: `split('</think>')[0]` (first piece — for reasoning) and `split('</think>')[-1]` (last piece — for remaining content). When content has multiple `</think>` tags, reasoning comes from before the first tag and remaining content comes from after the last tag. Everything between the first and last `</think>` tags is discarded — it is neither reasoning nor visible content. Without this distinction, a `</think>` tag would appear as literal text in the visible content portion of the prompt, a token sequence the model was never trained on. Grammar constraints prevent this when tools are active, but plain chat without tools has no grammar — the model generates freely and could produce multiple `</think>` tags. The fork uses `strings.Index` (first `</think>`, stored as `firstClose`) for the reasoning boundary and `strings.LastIndex` (last `</think>`, stored as `lastClose`) for the remaining content boundary, matching the official template's `[0]`/`[-1]` split semantics.

**Test 10 — Nested `<think>` tags.** `splitQwen35ReasoningContent("<think>outer<think>inner</think>more</think>content", "")` → reasoning `"inner"`, remaining `"content"`. The content has multiple `<think>` open tags before the first `</think>`. The official template's `split('<think>')[-1]` takes text after the LAST `<think>`, discarding text between earlier `<think>` tags ("outer" is lost). The fork's `strings.LastIndex(before, "<think>")` achieves the same: it finds the last `<think>` in `before` (at position 12, not 0), returning "inner" not "outer<think>inner". The text between the two `</think>` tags ("more") is silently discarded by both template and fork (`split('</think>')[0]` vs `split('</think>')[-1]` lose the middle segments). This test is the sole catcher of a mutation replacing `LastIndex` with `Index` for `<think>` — no other test has multiple `<think>` tags in `before`, so `LastIndex` and `Index` would return the same position in all other tests.

**Test 11 — No `<think>` open tag with multiple `</think>` close tags.** `splitQwen35ReasoningContent("inner</think>more</think>content", "")` → reasoning `"inner"`, remaining `"content"`. This combines the else branch (`open == -1`, line 79) with `lastClose != firstClose` (multiple `</think>` tags). Test 3 covers close-only with a single `</think>` (else branch, `lastClose == firstClose`). Test 9 covers multiple `</think>` with a `<think>` present (`open != -1`). This test covers the unique combination: no `<think>` AND multiple `</think>`. The official template's `split('<think>')[-1]` on a string without `<think>` returns the whole string, so reasoning is the full text before the first `</think>`. The fork's else branch does the same.

**Test 12 — Empty remaining content from Path 2 extraction.** `splitQwen35ReasoningContent("<think>reasoning</think>", "")` → reasoning `"reasoning"`, remaining `""`. The `</think>` tag is at the very end of content with nothing after it. All prior Path 2 tests produce non-empty remaining. This verifies that `TrimLeft("\n")` on an empty string works correctly, and covers the case where the assistant reasoned but produced no visible text (the response is entirely tool calls rendered separately by the tool call loop).

**Test 13 — `<think>` open tag present but NO `</think>` close tag.** `splitQwen35ReasoningContent("content<think>reasoning", "")` → reasoning `""`, remaining `"content<think>reasoning"`. Test 5 has no tags at all and reaches Path 3. This test has a `<think>` tag but no matching `</think>`, which must also reach Path 3: the entry condition at line 74 checks for `</think>` via `strings.Index`, not `<think>`. The content — including the literal `<think>` — passes through unchanged. This is the sole catcher of a mutation changing the entry condition to check for `<think>` instead of `</think>`.

**Test 14 — `</think>` at the very start of content (empty reasoning via Path 2).** `splitQwen35ReasoningContent("</think>content", "")` → reasoning `""`, remaining `"content"`. This reaches Path 2 with empty reasoning via extraction — not Path 3. The `before` variable is `""` (nothing before the first `</think>`), `open = LastIndex("", "<think>")` returns -1, so the else branch sets reasoning = before = "". After TrimSpace, reasoning is "". Tests 5 and 8 also produce empty reasoning, but they reach it via Path 3 (no `</think>` in content). This test verifies that Path 2's else branch produces correct empty reasoning when the close tag appears at position 0.

**Test 15 — Space after `</think>` preserved in remaining (newline-only stripping guard).** `splitQwen35ReasoningContent("<think>reasoning</think> content with leading space", "")` → reasoning `"reasoning"`, remaining `" content with leading space"`. The leading space (`0x20`) is preserved because both the template's `.lstrip('\n')` and the fork's `strings.TrimLeft(content, "\n")` strip **only newline characters** (`\n`, `0x0a`) — not spaces (`0x20`), not tabs (`\t`, `0x09`), not carriage returns (`\r`, `0x0d`), not any other whitespace. All prior Path 2 tests have only `\n` characters (or nothing) between `</think>` and the remaining content. This is the sole test with non-newline whitespace after `</think>`, and it catches three distinct mutations: (1) `TrimLeft("\n")` → `TrimSpace` (strips all whitespace including the space), (2) `TrimLeft("\n")` → `TrimLeft("\n ")` (strips newlines AND spaces), (3) `TrimLeft("\n")` → `TrimLeft("\n\t ")` (strips newlines, tabs, and spaces). Any of these would produce `"content with leading space"` instead of the correct `" content with leading space"`.

**Test 16 — Newline then space after `</think>` (strip boundary enforcement).** `splitQwen35ReasoningContent("<think>reasoning</think>\n content", "")` → reasoning `"reasoning"`, remaining `" content"`. The `\n` is stripped by `TrimLeft("\n")`, but the space (`0x20`) immediately after it is preserved. This enforces the exact byte-level boundary: the function strips consecutive `\n` characters from the front and stops at the first byte that is not `\n` — even if that byte is another whitespace character like space or tab. The template's `.lstrip('\n')` has identical semantics (Python's `lstrip` with an explicit character set strips only those characters).

**Implicit top-level content trimming coverage:** The renderer also applies `strings.TrimSpace` to all message content at `qwen35.go:139`, matching the template's `render_content(...)|trim` at line 82. Unlike the extraction path above (which strips only newlines), this top-level trim strips **all** whitespace (spaces, tabs, newlines, carriage returns, Unicode whitespace) from both ends — and the template's `|trim` does the same. This is not tested with explicit whitespace-laden content, but it is implicitly guarded by all 59 byte-exact subtests: any change to the top-level trim behavior would shift content bytes in the rendered output and break the byte-exact comparison against template ground truth.
...
**Status: DONE.** Implemented as `TestSplitQwen35ReasoningContent` in `model/renderers/qwen35_test.go` with 18 table-driven subtests plus an explicit cross-encoding equivalence assertion (19 subtests total). All test inputs use realistic content with `\n` around tags as the model actually generates, plus defensive pathological inputs for third-party client resilience, plus whitespace edge cases that enforce the exact stripping semantics. All expected values verified against the official Qwen 3.5 Jinja2 template by running the extraction algorithm directly in Python and rendering through the full template. The 18 subtests cover: (1a-c) path equivalence across three client encodings of the same model turn, (2) multi-line reasoning with internal newlines preserved, (3) close tag without open tag (model's raw output format), (4) explicit field wins over inline tags (no double-extraction), (5) no thinking baseline, (6) whitespace-only Thinking field, (7) empty content with explicit thinking (tool-call-only turn), (8) both empty, (9) multiple `</think>` tags — reasoning from before the first, remaining from after the last, (10) nested `<think>` tags — `LastIndex` finds innermost, (11) no `<think>` with multiple `</think>` — else branch + `lastClose != firstClose`, (12) empty remaining after extraction, (13) `<think>` without `</think>` — Path 3 despite open tag in content, (14) `</think>` at content start — empty reasoning via Path 2 not Path 3, (15) space after `</think>` preserved in remaining — enforces `TrimLeft("\n")` not `TrimSpace`, (16) newline-then-space after `</think>` — newline stripped, space preserved. The equivalence subtest calls the function three times with the three client encodings and asserts identical `(reasoning, remaining)` tuples. Test 9 uncovered a bug where the fork used `strings.Index` (first `</think>`) for both reasoning and remaining content; the fix (commit `ebf97712`) introduced `firstClose`/`lastClose` — `strings.Index` for reasoning and `strings.LastIndex` for remaining content, matching the official template's `split('</think>')[0]`/`split('</think>')[-1]` semantics. Tests 10-16 cover 7 previously uncovered code paths and whitespace edge cases in the function's decision tree, verified by mutation testing: all 5 code mutations (replacing `LastIndex` with `Index` for `<think>`, replacing `LastIndex` with `Index` for `</think>` remaining, changing entry condition from `</think>` to `<think>`, removing the else branch, and changing `TrimLeft("\n")` to `TrimSpace` for remaining content) are caught by at least one of the 7 new subtests. All 19 subtests pass.

---

## Gap 4: Tool Definition Serialization Tests — ALL KNOWN DIVERGENCES NOW HAVE TEST ENFORCEMENT
...
Note on Go's struct-based serialization: Go's `json.Marshal` emits struct fields in declaration order, which is deterministic regardless of what the client sent. This is actually **superior** to the official Jinja2 template's `{{ tool | tojson }}` passthrough, which blindly preserves whatever Python dict ordering the client happens to provide — if a client sends `description` before `type`, the template faithfully reproduces that non-training-distribution order. Go normalizes all inputs to struct field order. When that struct field order matches `get_json_schema()` insertion order (which IS the training data format), Go produces training-consistent output for every client. The sole remaining gap is `ToolProperty.Items`, which is typed as `any` — Go falls back to `map[string]any` and alphabetizes the keys instead of normalizing to training order.

The real multi-key `items` sub-objects come from complex element types via `_parse_type_hint()` recursion in HuggingFace Transformers' `get_json_schema()` function: `list[dict[str, int]]` → items = `{"type": "object", "additionalProperties": {"type": "integer"}}` (2 keys), `list[Optional[int]]` → items = `{"type": "integer", "nullable": true}` (2 keys). These are verified outputs from `get_json_schema()` in HuggingFace Transformers v5.3.0, confirmed by running the official `Qwen/Qwen3.5-27B` tokenizer's `apply_chat_template` method (verified with a script that calls `get_json_schema()` on Python functions with these type hints and renders through `apply_chat_template`). Go's `json.Marshal` alphabetizes `map[string]any` keys, producing `{"additionalProperties": ..., "type": ...}` instead of the training-consistent `{"type": ..., "additionalProperties": ...}`. The fix is to change `Items any` to `Items *ToolProperty` in the `ToolProperty` struct in `api/types.go`, so Go's struct field ordering applies recursively at every nesting level — see the corrected struct below.

All known tool definition divergences now have test enforcement (2026-04-03):

1. **`enum`/`description` field ordering in `ToolProperty`** — Tool 2 (`choose_color`) FAILS by design. Go's struct declares `Description` before `Enum`; HF produces `enum` before `description`.
2. **Multi-key `items` sub-object key ordering** — Tool 3 (`search_records`) FAILS by design. `Items any` → `map[string]any` → `json.Marshal` alphabetizes; HF preserves insertion order (`type` before `additionalProperties`).
3. **Silent field loss during `json.Unmarshal`** — three `silent_field_loss` subtests FAIL by design: `nullable` (`Optional[str]`), `additionalProperties` (`dict[str, int]`), `prefixItems` (`tuple[int, int]`). Each subtest constructs the tool via JSON unmarshal (the real client path) and accepts either correct rendering or an error; fails only on silent corruption. Unlike informational metadata (e.g., the `"return"` field that `get_json_schema()` also produces, which is harmless to drop because both with-and-without variants are in-distribution and carry no constraint semantics), these fields carry **type constraint semantics** — `"nullable": true` means "accepts null" and is a fundamentally different schema from one without it. A developer using `Optional[int]` in Python has no way to avoid this: HuggingFace Transformers' `get_json_schema()` function adds `"nullable": true` automatically, and Ollama drops it on unmarshal with no error.

A third divergence — **scalar boolean/None capitalization** (`true`/`false`/`null` instead of `True`/`False`/`None`) — affects tool call **arguments** (the `formatToolCallArgument` function in `model/renderers/qwen3coder.go`), not tool **definitions** (the `marshalWithSpaces` function in `model/renderers/json.go`). It is a different code path with a different failure mode: tool call arguments are round-tripped through the parser and re-serialized in historical assistant messages, causing KV cache invalidation (failure mode 2) in addition to training distribution shift (failure mode 1). This divergence is fully specified in Gap 10.

### What the correct test must verify — with byte-exact HuggingFace ground truth

The following ground truth strings were generated by running HuggingFace Transformers v5.3.0 `get_json_schema()` + `_compile_jinja_template()` on the official `Qwen/Qwen3.5-27B` `chat_template.jinja` template (the `chat_template` field from the HuggingFace model's `tokenizer_config.json`), using Python functions with type hints that exercise each serialization property. Every byte was verified by saving both the HuggingFace Transformers output and the Go `Qwen35Renderer` output to files and running `diff`.
...
Go `Qwen35Renderer` output for the same tool: **byte-identical**. Verified by `diff` — zero differences. **Test implemented and passing** — `TestQwen35RendererToolDefinitionsMatchOfficialTemplate` compares the extracted tool JSON line byte-exact against this ground truth string.
...
Root cause: HuggingFace Transformers' (`transformers` @ `3a3b59c`) `get_json_schema()` in `chat_template_utils.py` assigns `schema["enum"]` before `schema["description"]`. Python dicts preserve insertion order, and HuggingFace Transformers' `tojson` override (the `_custom_tojson` function in the same file) uses `json.dumps(sort_keys=False)`, so the training data has `enum` before `description`. Go's `ToolProperty` struct in `api/types.go` declares `Description` (field 4) before `Enum` (field 5), and `json.Marshal` emits struct fields in declaration order.
...
### What the test looks like — ALL TOOLS IMPLEMENTED

The test uses **realistic tool definitions** that match what HuggingFace Transformers' `get_json_schema()` actually produces. Each tool exercises one specific serialization property. Failure messages tell the implementer exactly what to change.

**Tool 1 (get_weather) passes.** **Tool 2 (choose_color) FAILS** (enum/description ordering). **Tool 3 (search_records) FAILS** (multi-key items ordering). **Three silent_field_loss subtests FAIL** (nullable, additionalProperties, prefixItems). See the actual test code in `model/renderers/qwen35_test.go`. The example code below shows the struct layout for reference.
...
func TestQwen35RendererToolDefinitionsMatchOfficialTemplate(t *testing.T) {
    ...
    if gotTool2 != wantTool2 {
        t.Errorf("Tool 2 (choose_color) enum/description field ordering "+
            ...
            "enum before description (in chat_template_utils.py, the "+
            "schema[\"enum\"] assignment precedes schema[\"description\"]). "+
            "Python dicts preserve insertion order, and HF's "+
...
**Cases 4–5: Key ordering inside nested `items` sub-objects** — HuggingFace Transformers' `get_json_schema()` produces multi-key `items` sub-objects for complex element types (empirically verified by running `get_json_schema()` on these type hints and rendering through `apply_chat_template`). Go's current `Items any` field causes `json.Unmarshal` to parse items into `map[string]any`, and `json.Marshal` alphabetizes map keys — producing wrong key ordering. The fix (`Items *ToolProperty` instead of `Items any`) makes `json.Marshal` emit keys in struct declaration order at every nesting level recursively.
...
func TestQwen35ToolPropertyRoundTripFidelity(t *testing.T) {
    cases := []struct {
        ...
        // HF's _parse_type_hint(Optional[int]) in chat_template_utils.py
        // adds "nullable": true to the property dict. Go's
        ...
        // HF's _parse_type_hint(tuple[str, int]) in chat_template_utils.py
        // produces {"type": "array", "prefixItems": [{"type":
        ...
        // Empirically verified via get_json_schema() + apply_chat_template.
        {
...
**Real issue — OPEN (code fix), test coverage partially DONE.** The old test's out-of-training-distribution `items` example has been replaced with realistic HF ground truth (2026-04-01). The rewritten `TestQwen35RendererToolDefinitionsMatchOfficialTemplate` now uses single-key `items` (`list[str]` → `{"type": "string"}`) which passes. Multi-key `items` from complex element types still need the `ToolProperty` struct rewrite to fix — tested by the proposed `TestQwen35ToolPropertyRoundTripFidelity` (cases 4-5). HuggingFace Transformers' `get_json_schema()` produces multi-key `items` sub-objects for complex element types:
...
This was empirically verified by running `get_json_schema()` on Python functions with these type hints, then rendering through the official `Qwen/Qwen3.5-27B` tokenizer's `apply_chat_template`. The rendered prompt contains the multi-key items objects with `type` first (Python insertion order).
...
**What is missing:** Tests that include multiple tool calls in one assistant turn (`TestQwen35RendererBackToBackToolCallsAndResponses` — byte-exact with think subtests, `TestQwen35RendererInterleavedThinkingAndTools` — byte-exact with think subtests) use only scalar arguments (strings and integers). No test combines scalar arguments in one tool call with structured JSON arguments (maps or arrays) in another tool call within the same assistant message.

**Why this matters:** In real agentic use, the model frequently calls multiple tools in one turn with different argument types. For example: `get_weather(location="Paris")` (string scalar) alongside `filter(criteria={"temp_min": 20, "unit": "celsius"})` (JSON object). The `formatToolCallArgument` function in `model/renderers/qwen3coder.go` handles scalars and structured types through different code paths (strings are returned verbatim, maps go through `marshalQwenToolCallArgument` which uses `json.Encoder` with `SetEscapeHTML(false)` and space insertion). A test with mixed types in parallel tool calls would exercise both paths in a single render call and verify they don't interfere with each other.
...
## Gap 9.5: Tool Call Argument Object Key Ordering Is Destroyed During Round-Trip

**What is wrong:** When the model generates a tool call with a JSON object argument like `{"z": 1, "a": 2}`, the key ordering is destroyed during the parser→storage→renderer round-trip, and the re-rendered prompt contains `{"a": 2, "z": 1}` (alphabetized). This invalidates the KV cache from the divergence point onward (failure mode 2). The problem compounds in multi-turn agentic conversations: every tool call with an object argument contributes a cache miss point.

**The three-stage key order collapse:**

1. **Parser stage** (`parseValue()` in `model/parsers/qwen3coder.go`): The parser extracts the raw argument string `{"z": 1, "a": 2}` from the XML tags, then calls `json.Unmarshal([]byte(raw), &obj)` into a plain Go `map[string]any`. Go maps are unordered — the original key ordering from the model's output is destroyed at this point.

2. **Client round-trip stage** (`ToolCallFunctionArguments` in `api/types.go`): The top-level argument names are stored in an insertion-ordered map (`orderedmap.Map[string, any]`), preserving the order of parameter names. But the *values* are `any` — when a value is a parsed JSON object, it is a plain `map[string]any` with no ordering. The client receives this object, serializes it to JSON (Go's `json.Marshal` alphabetizes map keys), and sends it back. The server deserializes — still unordered.

3. **Renderer stage** (`marshalQwenToolCallArgument()` in `model/renderers/qwen3coder.go`): The renderer calls `json.NewEncoder().Encode(value)` on the `map[string]any`. Go's JSON encoder alphabetizes map keys. The re-rendered argument is `{"a": 2, "z": 1}` — different bytes from the model's original `{"z": 1, "a": 2}`.

**KV cache round-trip tests confirm the failure.** Three test cases in `server/qwen35_kvcache_roundtrip_test.go` set `expectToolCallBodyRoundTrip: true` and `expectFullAssistantRoundTrip: true` for non-canonical key ordering, and all three **fail**:

- Case 10 ("noncanonical key order"): `{"z": 1, "a": 2}` → re-rendered as `{"a": 2, "z": 1}`. 50/102 bytes reused.
- Case 11 ("nested key order drift"): `{"outer": {"z": 1, "a": 2}, "b": 3}` → re-rendered as `{"b": 3, "outer": {"a": 2, "z": 1}}`. Keys alphabetized at every nesting depth.
- Case 15 ("real client resend via JSON roundtrip and user tool_response"): Same key order failure through the full client round-trip path.

**Additionally, two test cases fail on number normalization** — a related round-trip fidelity issue in the same `marshalQwenToolCallArgument` code path:

- Case 12 ("number lexical form 1.0"): Model generates `1.0`, Go's `json.Unmarshal` parses to `float64(1)`, re-serialized as `1`. Four bytes lost.
- Case 13 ("number lexical form 1e3"): Model generates `1e3`, re-serialized as `1000`. Different byte sequence, same numeric value.

**How llama.cpp solves this:** llama.cpp uses `nlohmann::ordered_json` — a JSON parser that preserves insertion order using an ordered map internally (`std::vector<std::pair<key, value>>` for ordered iteration, `std::unordered_map` for O(1) lookups). When `ordered_json::parse()` processes `{"z": 1, "a": 2}`, the key order `z, a` is preserved. The parsed object is converted to minja's `value_object_t` (also insertion-ordered), and the Jinja2 template's `tojson` filter iterates in insertion order via `as_ordered_object()` (explicitly commented `// IMPORTANT: need to keep exact order` in `common/jinja/value.cpp`'s `tojson` implementation). The round-trip preserves key ordering. llama.cpp still has the same number normalization issue (`1.0` → `1`) because `nlohmann::json` normalizes number representations during parsing.

**What the fix requires in Go:** The `parseValue()` function in `model/parsers/qwen3coder.go` must use an insertion-order-preserving JSON parser for object values instead of `json.Unmarshal` into `map[string]any`. Go's standard library does not provide one. Options include using the `orderedmap` package already present in the fork (`internal/orderedmap/orderedmap.go`, which wraps `github.com/wk8/go-ordered-map/v2`) or using `json.Decoder` with `Token()` to manually parse key-value pairs in order. The `marshalQwenToolCallArgument` function would then need to serialize ordered maps in insertion order instead of relying on `json.Encoder`'s alphabetical map key sorting. Storing raw argument strings (to re-emit them verbatim) is not viable because it would break the API contract — clients would receive `"payload": "{\"z\": 1}"` (a JSON string containing JSON) instead of `"payload": {"z": 1}` (a nested JSON object).

**Think mode variants:** Not needed. Tool call argument serialization is handled by `formatToolCallArgument` and `marshalQwenToolCallArgument` in `model/renderers/qwen3coder.go`, which operate on Go values and have no access to think mode state.
...
**Scope note:** This fix is Qwen-renderer-local. The `formatToolCallArgument` function has exactly two production callers: `qwen35.go:168` (model `"qwen3.5"`) and `qwen3coder.go:187` (model `"qwen3-coder"`). No other renderer calls it — the Qwen3VL renderer uses `marshalWithSpaces` on the whole arguments object (JSON format), not per-argument XML. Both the Qwen 3.5 and Qwen3-Coder official Jinja2 templates use byte-identical argument rendering logic at line 122 and line 89 respectively: `args_value | string` for scalars, `args_value | tojson | safe` for collections. Jinja2's `| string` calls Python `str()`, verified by running the filter in isolation (`true | string` → `"True"`) and by rendering both full templates with boolean arguments (both produce `True`/`False`/`None`). Ground truth saved to `/tmp/formatToolCallArgument_ground_truth.md`.
...
**`TestFormatToolCallArgumentThinkingVL` deleted.** This test in `qwen3vl_thinking_test.go` tested `formatToolCallArgument` — a function the Qwen3VL renderer does not call in production (VL uses `marshalWithSpaces` on the whole arguments object, a JSON path, not per-argument XML). The test was a 5-case subset of our 11-case test, with wrong expected values for booleans (`"true"` instead of `"True"`). It would have blocked the `formatToolCallArgument` fix by failing when the function is corrected. There are exactly two production callers of `formatToolCallArgument`: `qwen35.go:168` and `qwen3coder.go:187`. No VL code calls it.

**Think mode variants:** Not needed for the `formatToolCallArgument` code change itself — tool call argument formatting is independent of think mode.

---

## Gap 11: No Test for `lastQueryIndex` Edge Cases — EXPANDED with boundary enforcement (Property 2)
...
**Normal boundary testing** — now also specified in **Gap 13** (pre-lastQueryIndex reasoning discarding) and **Gap 12 subtest 5** (multi-turn tool history with boundary). These test the positional `<think>` wrapping boundary (Property 2) in realistic scenarios where the boundary matters.

**Edge cases — divergence from the official template:** The official template rejects these inputs with `raise_exception()`. The Go renderer silently proceeds and produces wrong output — `lastQueryIndex` defaults to `len(messages) - 1`, which means no assistant messages get `<think>` wrapping even when they should. This is not a deliberate design choice; it is a missing validation check. The `input_validation_report.md` specifies the fix: 6 one-liner checks inside `Render()`, including check #3 (no real user query) which covers these edge cases. Once validation is added, these become error-path tests — the test asserts `Render()` returns an error, not that it produces specific wrong output.

**Design decision (2026-04-03):** The fork should be at least as strict as the official template, and stricter where the template is too permissive (e.g., empty tool function names produce `<function=>` — a training-absent token sequence the template doesn't reject but the fork should). Verified that adding validation would not break any existing test — all 59 byte-exact subtests use valid message arrays.

**What the tests must verify:** Each edge case asserts that `Render()` returns an error (not silent wrong output):

1. **All user messages are `<tool_response>`-wrapped.** Must return error, not silently set wrong `lastQueryIndex`.

2. **No user messages at all (only system + assistant).** Must return error, not silently render with wrong wrapping boundary.

3. **System message not at index 0.** Must return error, not silently render it as a `<|im_start|>system` block in the middle of the conversation.

4. **Unknown message role (e.g., `"function"`).** Must return error, not silently skip the message.

5. **Empty tool function name.** Must return error, not produce `<function=>` (stricter than the template, which lets this through).

6. **No messages at all.** Must return error, not return empty string with nil error.

**Think mode variants:** Both think=true and think=false for each error case, verifying the error is returned regardless of think mode.
...
**Status dependency:** Covered by Gap 12 subtests 5 and 6. Gap 12 implementation satisfies this gap — the same assertions enforce both the prefill property and the reasoning discarding property in the same subtests.
...
|----------|-------------------|-----------|
| P1 Think-independence | Gap 2 (InterleavedThinkingAndTools/think=false); **Gap 12** subtests 2, 4, 5 (prefill path); ToolDefinitionsMatchOfficialTemplate sysprompt invariant | **Gap 5** (KV cache level) |
| P2 Positional wrapping | BackToBackToolCallsAndResponses (pre-LQI); InterleavedThinkingAndTools (post-LQI); **Gap 12** subtests 5, 6 (both sides byte-exact); **Gap 13** (covered by Gap 12) | — |
| P3 Prefill | **Gap 12** (6 subtests); AssistantToolCallIsNotPrefill (negative) | — |
| P4 Path equivalence | SplitQwen35ReasoningContent (unit — 19 subtests); **AssistantPrefillWithThinking/path_equivalence** (integration — 3 encodings, same `wantWithReasoning`) | — |
| P5 Arg formatting | FormatToolCallArgumentMatchesOfficialTemplate (14 cases, 13✓ 1**F**); Gap 12 subtest 5 (str+bool+int+nil in tool history) | — |
| P6 Tool def serialization | ToolDefinitionsMatchOfficialTemplate (Tool 1 ✓; Tool 2 **FAILS** enum/desc; Tool 3 **FAILS** items ordering; 3 field loss subtests **FAIL** nullable/additionalProperties/prefixItems) | **Gap 4** (struct rewrite fixes all); Gap 6 (KV cache) |
| P7 Tool response grouping | 4 tests: grouped, non-consecutive, single, Gap 12 subtest 5 (tool history) | — |
| P8 Generation prompt | All 9 think=false tests + NoThinkPrefill | — |

### Gap status and relationships

**Closed:** Gaps 1, 2, 3, 10, **12**, **13** (6 gaps). Gaps 1 and 2 protect the fork's most important correctness fixes. Gap 3 tests the extraction function in isolation. Gap 10 fixed scalar bool/None/int capitalization. Gap 12 upgraded the prefill test to 6 subtests enforcing Properties 1, 2, 3 (commit `232e3b97`). Gap 13 covered by Gap 12 subtests 5 and 6.

**Partially closed:** Gap 4 (tool definition serialization). All known divergences now have test enforcement: Tool 1 passes; Tool 2 FAILS (enum/description ordering); Tool 3 FAILS (multi-key items ordering); 3 silent field loss subtests FAIL (nullable, additionalProperties, prefixItems). All require the `ToolProperty` struct rewrite in `api/types.go`. System prompt think-independence assertion added (commit `664bcdf7`).

**Restructured:** Gap 11 (input validation error paths). Boundary testing covered by Gap 12. The remaining edge cases (all-tool-response users, no user messages, system not at index 0, unknown roles, empty function names, empty messages) are now specified as error-path tests — `Render()` must return an error, not silent wrong output. Design decision made: the fork is at least as strict as the template, and stricter on empty function names. See `input_validation_report.md` for the 6-check implementation. Verified that no existing test would break.

**Open serialization and round-trip gaps:**

- **Gap 4** (ToolProperty struct rewrite) — 5 test failures (Tool 2, Tool 3, 3 field loss subtests) require the `ToolProperty` struct rewrite in `api/types.go`.

- **`very_large_int_float` int64 overflow** (independent of Gap 4) — `FormatToolCallArgumentMatchesOfficialTemplate/very_large_int_float` FAILS because `strconv.FormatInt(int64(float64(1e20)))` overflows, producing `"-9223372036854775808"` instead of `"1e+20"`. Fix: add `v >= math.MinInt64 && v <= math.MaxInt64` guard before the `int64` conversion in `formatToolCallArgument` in `model/renderers/qwen3coder.go`. This is a one-line code fix with no dependency on the struct rewrite — it can be applied independently at any time.
...
The coverage matrix shows think mode coverage visually. **Every property-enforcing test must have think subtests unless the property is provably think-independent at the code level** (e.g., Property 5 argument formatting operates on Go values with no access to think state). All property-enforcing renderer tests now have think subtests — the P1 column shows ✓/✓ across all cells. The `ToolDefinitionsMatchOfficialTemplate` test additionally asserts system prompt byte-identity across think modes. Total: 10 test functions, 59 subtests across both files (qwen35_test.go: 8 renderer tests with 26 subtests + SplitQwen35ReasoningContent with 19 subtests; qwen3coder_test.go: FormatToolCallArgument with 14 subtests).

### Not-broken-but-untested items

NBU-1 through NBU-4 are **resolved** (Gap 12, commit `232e3b97`). NBU-5 is **resolved** (ToolDefinitions upgrade, commit `664bcdf7`). No unresolved NBU items remain.
