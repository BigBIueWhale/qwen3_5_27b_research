...
**`TestQwen35RendererBackToBackToolCallsAndResponses`**: Byte-exact comparison of the entire rendered output for a multi-turn agentic conversation with two tools (`add` and `multiply`) defined in the system prompt, an assistant message with back-to-back tool calls, grouped tool responses, and a follow-up user query. The expected strings were derived by tracing through the official Qwen/Qwen3.5-27B Jinja2 chat template with the same message array and cross-checked against actual Go renderer output via diff. Has `think=true` and `think=false` subtests. The assistant message at index 2 is before `lastQueryIndex=5` (the "Summarize the results." user message), so the official template discards reasoning content and renders without `<think>` wrapping (template line 102-103) — this is a position-based rule, NOT controlled by `enable_thinking`. A targeted diagnostic checks that the thinking content "Need to call add and multiply." does not leak into the output, with an error message explaining the lastQueryIndex mechanism. The byte-exact comparison locks in: tool definition JSON for two tools (field ordering, spaced separators), the `\n\n` separator between content and the first tool call (template line 112), the `\n` separator between subsequent tool calls (template line 117), tool response grouping under a single `<|im_start|>user` block, the `<|im_end|>` closure after tool calls, and the generation prompt suffix for both think modes. Tool call arguments are integers (2, 3, 4, 5) — `formatToolCallArgument` produces identical output for both Go `int` and JSON-deserialized `float64` values for small whole numbers. This test complements `TestQwen35RendererInterleavedThinkingAndTools`, which tests the opposite scenario: assistant messages AFTER lastQueryIndex where thinking IS preserved.
