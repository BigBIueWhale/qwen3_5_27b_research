...
**Relationship to Gap 2 — same fix, different test level:** This function's signature was changed as part of the same fix described in Gap 2. The upstream version (commit `9896e36`) has the signature `splitQwen35ReasoningContent(content, messageThinking string, isThinking bool)` and gates path 1 on `if isThinking && messageThinking != ""`. The fork removed the `isThinking` parameter entirely, changing the signature to `splitQwen35ReasoningContent(content, messageThinking string)` and the guard to `if messageThinking != ""`. This means the upstream version silently discards stored reasoning when `isThinking` is false — the reasoning never reaches the rendering code at line 143. Gap 2 tests the renderer-level consequence of this change (rendering with `think=false` and verifying thinking blocks appear in history). This gap tests the function's internal code paths in isolation — ensuring each extraction path produces the correct output regardless of how the renderer uses it. Both gaps are needed: Gap 2 catches the regression at the integration level (someone re-adds `isThinking` gating), while this gap catches bugs in the extraction logic itself (for example, double-wrapping, incorrect tag stripping, or wrong fallback behavior).
...
**Think mode variants:** Not needed. In the fork's version, `splitQwen35ReasoningContent` is a pure function that takes `content` and `messageThinking` strings and returns `reasoning` and `remaining` strings. It has no access to `isThinking`, `ThinkValue`, or any renderer state — the `isThinking` parameter was deliberately removed as part of the fix described in Gap 2. Its behavior is identical regardless of think mode. The think mode only affects how the renderer uses the function's output — specifically, whether the generation prompt at the end of the rendered output includes `<think>\n` or `<think>\n\n</think>\n\n` — but that is tested separately in the generation prompt suffix assertions of other tests. (In the upstream version, the function DOES take `isThinking` and its behavior DOES change with think mode — but testing the upstream signature is not the purpose of this gap; that regression is caught by Gap 2's renderer-level test.)
...
**Gaps 2 and 3 are two levels of the same fix.** The fork's unconditional thinking block fix involved two code changes: removing the `isThinking` parameter from `splitQwen35ReasoningContent` (line 65, tested by Gap 3) and removing the `isThinking &&` gate from the rendering condition (line 143, tested by Gap 2). Gap 2 tests the renderer-level consequence — rendering with `think=false` and verifying that historical thinking blocks appear in the output. This catches regressions to either part of the fix, because re-adding `isThinking` gating to the extraction (Part 1) causes the reasoning text to vanish from inside the `<think>` tags, while re-adding it to the rendering condition (Part 2) causes the `<think>` tags themselves to vanish. Gap 3 tests the extraction function's internal code paths in isolation — ensuring each path (explicit `Thinking` field, inline `<think>` tags in content, no thinking) produces the correct output. Gap 2 should be implemented first because it catches both regression vectors with a single test; Gap 3 adds depth by verifying the extraction logic independent of how the renderer uses it.
