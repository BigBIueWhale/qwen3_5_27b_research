...
**What the bug is:** The test at `model/renderers/qwen35_test.go` line 259 encodes three distinct field ordering deviations from the official Qwen 3.5 training data, all stemming from how Go serializes the `api.Tool` type hierarchy versus how HuggingFace's `get_json_schema()` constructs Python dicts (which `tojson` then serializes with insertion-order preservation, `sort_keys=False`):

1. **`ToolFunctionParameters`: `required` before `properties` (should be reversed).** The Go struct declares fields as `Type`, `Defs`, `Items`, `Required`, `Properties`. HuggingFace's `_convert_type_hints_to_json_schema()` at `chat_template_utils.py:210-213` constructs `{"type": "object", "properties": properties}` first, then appends `schema["required"] = required`. The model was trained on `properties` before `required`.

2. **`ToolProperty` (with enum): `description` before `enum` (should be reversed).** The Go struct declares `Description` before `Enum`. HuggingFace's `_parse_type_hint()` at `chat_template_utils.py:139-142` constructs `{"type": ..., "enum": [...]}` first, then `get_json_schema()` at line 380 appends `schema["description"] = desc` last. The model was trained on `enum` before `description`.

3. **`ToolProperty.Items` (as `map[string]any`): keys alphabetized (should preserve insertion order).** The test's `Items: map[string]any{"type": "string", "description": "..."}` gets serialized by Go as `{"description": "...", "type": "string"}` (alphabetical). Python's `_parse_type_hint()` produces `{"type": "string"}` first, and descriptions are added after, giving `{"type": "string", "description": "..."}`. The model was trained on `type` before `description`.

**Why this matters for the test suite specifically:** The test at line 259 hardcodes the expected JSON as:
```
"parameters": {"type": "object", "required": ["location"], "properties": {"location": {...}, "filters": {"type": "array", "items": {"description": "...", "type": "string"}}}}
```
This locks in all three deviations. If someone fixes any of the orderings, the test will **fail** and signal a false regression. The test is actively preventing the fix.

**What needs to change:** The test's expected JSON string at line 259 should use the HuggingFace training data order: `properties` before `required`, `enum` before `description`, and `type` before `description` inside `items`. This change should be made at the same time as the actual fix to the serialization, so that the test transitions from locking in the wrong behavior to locking in the correct behavior.

**Note on blast radius:** `ToolFunctionParameters` and `ToolProperty` are shared types used by every tool-calling renderer in Ollama. Changing their marshaling behavior requires verifying that all other model families also expect the same field ordering, or implementing renderer-local custom marshaling. The `Items` fix is harder because `Items` is typed as `any` — Go loses insertion order when deserializing JSON maps into `map[string]any`. This requires either an ordered-map type (like `ToolPropertiesMap`) or renderer-local marshaling. Beyond ordering, `ToolProperty` also lacks three fields that `get_json_schema()` can produce: `nullable` (`Optional[T]`, at `chat_template_utils.py:127`), `additionalProperties` (`Dict[str, T]`, line 175), and `prefixItems` (`Tuple[T1, T2]`, line 168) — these are silently dropped during deserialization (see `consolidated_report.md` Section 4.2 for the full analysis). This test's tools don't exercise those types, but any ordering fix should also address field completeness. This verification work is documented in `consolidated_report.md` Part 4 (Research topics 1-5). The test change should wait until the verification is complete and the fix is implemented.
...
**Gaps 2 and 3 are two levels of the same fix.** The fork's unconditional thinking block fix involved two code changes: removing the `isThinking` parameter from `splitQwen35ReasoningContent` (line 65, tested by Gap 3) and removing the `isThinking &&` gate from the rendering condition (line 143, tested by Gap 2). Gap 2 catches the regression at the integration level (someone re-adds the `isThinking` gate). Gap 3 catches bugs in the extraction logic itself (wrong tag stripping, wrong fallback priority, wrong whitespace handling, broken path equivalence). Both are now closed.
