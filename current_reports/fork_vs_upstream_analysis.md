...
**Verification update (2026-03-11):** The shared `marshalWithSpaces` fix is still **not implemented** in the fork, but it has now been verified as **correct in principle** for all current runtime callers of that helper. Official HuggingFace templates downloaded for `Qwen/Qwen3.5-*`, `Qwen/Qwen3-VL-*`, `allenai/Olmo-3-7B-Instruct`, and `allenai/Olmo-3.1-32B-Instruct` all use `tools | tojson` for tool definitions, so the shared helper should emit literal `<>&`, not HTML-escaped `\u003c\u003e\u0026`, for `qwen3.5`, `qwen3-vl`, and `olmo3` tool-definition paths.
...
|----------|------|--------|---------|--------|
| **P0** | Fix remaining HTML escaping in shared `marshalWithSpaces` (`json.go`) | Small-Medium — the Qwen-local `formatToolCallArgument` path is now fixed. Caller verification is now complete for the helper's current runtime users (`qwen3.5`, `qwen3-vl`, `olmo3`): their official templates use `tools | tojson`, so disabling HTML escaping is correct in principle. Reference: Ollama's own `LFM2Renderer` at [`lfm2.go:55-56`](https://github.com/ollama/ollama/blob/82848a78/model/renderers/lfm2.go#L55-L56). | 3.1 Bug A | **OPEN** — verified correct for current helper callers, but still unimplemented. |
...
|-------|--------|-----|---------|
| **JSON serialization mismatches** in tool serialization — remaining issues are shared HTML escaping in `marshalWithSpaces` and key ordering in `ToolFunctionParameters`. The fork has already fixed compact separators and Qwen-local HTML escaping in `formatToolCallArgument`, but tool definitions still differ from HuggingFace's `tojson` override. The shared helper change has now been verified as correct in principle for its current runtime callers (`qwen3.5`, `qwen3-vl`, `olmo3`), but is still not implemented. | Remaining distribution shift on tool definitions and nested object ordering. | Replace `marshalWithSpaces` with a verified no-escape variant and solve key ordering separately. | 3.1, 3.4, 2.1 |
