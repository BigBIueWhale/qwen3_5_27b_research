...
|----------|------|--------|---------|--------|
| **P0** | Fix remaining HTML escaping in shared `marshalWithSpaces` (`json.go`) | Small-Medium — the Qwen-local `formatToolCallArgument` path is now fixed, but the shared serializer still uses `json.Marshal` and needs caller verification before a global change. Reference: Ollama's own `LFM2Renderer` at [`lfm2.go:55-56`](https://github.com/ollama/ollama/blob/82848a78/model/renderers/lfm2.go#L55-L56). | 3.1 Bug A | **OPEN** — empirically verified for shared serializer callers. |
...
|-------|--------|-----|---------|
| **JSON serialization mismatches** in tool serialization — remaining issues are shared HTML escaping in `marshalWithSpaces` and key ordering in `ToolFunctionParameters`. The fork has already fixed compact separators and Qwen-local HTML escaping in `formatToolCallArgument`, but tool definitions still differ from HuggingFace's `tojson` override. | Remaining distribution shift on tool definitions and nested object ordering. | Replace `marshalWithSpaces` with a verified no-escape variant and solve key ordering separately. | 3.1, 3.4, 2.1 |
