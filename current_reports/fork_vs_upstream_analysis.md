...
| JSON tool serialization (remaining key ordering + shared HTML escaping bugs; local tool-call argument spacing fixed 2026-03-10) | The fork now fixes the Qwen-local `formatToolCallArgument` path: structured tool call arguments render with spaced JSON and literal `<>&`. Remaining open issues are shared `marshalWithSpaces` HTML escaping and `ToolFunctionParameters` key ordering. Upstream still has the full original bug set. | Upstream still has the full bug; fork is partially fixed | Fork over upstream for Qwen-family tool call arguments; neither fully complete vs HuggingFace `tojson` because key ordering and shared serializer work remain open |
...
**Remaining JSON serialization gaps**: The JSON blob produced by `marshalWithSpaces(tool)` still differs from what HuggingFace's `tojson` override produces. As of 2026-03-10, the fork has fixed the Qwen-local `formatToolCallArgument` path for structured tool call arguments, but shared tool-definition serialization and key ordering remain open. See section 3.1 for the full verified analysis.
...
### 3.1 JSON serialization mismatches — remaining HTML escaping and key ordering, plus Sub-bug C now fixed locally in the fork
...
**Verification update (2026-03-11):** The shared tool-definition HTML-escaping fix is still **not implemented** in the fork, but it has now been verified as **correct in principle** for all current runtime callers of `marshalWithSpaces`. Official HuggingFace templates downloaded for `Qwen/Qwen3.5-*`, `Qwen/Qwen3-VL-*`, `allenai/Olmo-3-7B-Instruct`, and `allenai/Olmo-3.1-32B-Instruct` all use `tools | tojson` for tool definitions, so those paths should emit literal `<>&`, not HTML-escaped `\u003c\u003e\u0026`, for `qwen3.5`, `qwen3-vl`, and `olmo3`. **Critical implementation correction:** a failed attempt to land this on 2026-03-11 showed that changing `marshalWithSpaces` alone is insufficient. Nested tool-definition fields still flow through custom `MarshalJSON` paths in `api/types.go`, so a real fix must cover the entire tool-definition serialization graph for escaping, not only the top-level helper.
...
**Bug C — Compact separators in `formatToolCallArgument` (MEDIUM impact) — FIXED IN FORK 2026-03-10:**

HuggingFace's `tojson` uses Python's default separators `(', ', ': ')` — JSON with spaces after `:` and `,`. The existing `marshalWithSpaces` function at `json.go:8-45` correctly handles this for tool **definitions** in `Qwen35Renderer`. The fork now applies the same spacing convention, with HTML escaping disabled, in `formatToolCallArgument` for structured tool call arguments. This fix is scoped to the Qwen-family renderer path and does not yet change shared serializer infrastructure.
...
Previous fork behavior:      {"unit":"\u003ccelsius\u003e","verbose":true}  ← compact, HTML-escaped
Current fork behavior:       {"unit": "<celsius>", "verbose": true}         ← spaced, literal `<`
```
...
1. **Tool-definition escaping path, starting at `marshalWithSpaces`**: The originally proposed change was to replace the inner `json.Marshal` call with `json.NewEncoder` using `SetEscapeHTML(false)` (fixes Bug A at the top level). After failed implementation testing on 2026-03-11, the correct scope is now known to be broader: the patch must also cover nested tool-related custom `MarshalJSON` implementations in `api/types.go` so nested property descriptions stop HTML-escaping too. The spacing logic already handles Bug C for the top-level helper path. For Bug B, the `Required` and `Properties` field declarations in `api/types.go:486-491` remain a separate question.
2. **`formatToolCallArgument` in `qwen3coder.go`**: DONE in the fork on 2026-03-10. The function now uses a Qwen-local serializer with `SetEscapeHTML(false)` plus spaced separators, fixing Bugs A and C for tool call arguments without changing shared serializer code.
...
|----------|------|--------|---------|--------|
| **P0** | Fix remaining shared HTML escaping in the full tool-definition serialization path (starts at `marshalWithSpaces`, but does not end there) | Medium — the Qwen-local `formatToolCallArgument` path is now fixed. Caller verification is complete for current `marshalWithSpaces` users (`qwen3.5`, `qwen3-vl`, `olmo3`): their official templates use `tools | tojson`, so disabling HTML escaping is correct in principle. But failed implementation testing showed nested tool-definition fields still escape through custom `MarshalJSON` paths in `api/types.go`, so this patch must cover the full serialization graph, not just `json.go`. Reference: Ollama's own `LFM2Renderer` at [`lfm2.go:55-56`](https://github.com/ollama/ollama/blob/82848a78/model/renderers/lfm2.go#L55-L56). | 3.1 Bug A | **OPEN** — verified correct for current callers, but implementation scope is broader than helper-only. |
...
| ~~**P1**~~ **DONE** | Fix compact separators in `formatToolCallArgument` | Small — implemented in the fork on 2026-03-10. Structured tool call arguments now use spaced JSON, and this local path also disables HTML escaping. | 3.1 Bug C | **DONE** |
...
All other previously-open items (P0: JSON tool definitions, P0: tools-first ordering, P2: image support, P3: default system message, P3: unconditional think blocks, P3: lastQueryIndex tool_response filtering) are now **RESOLVED** by the dedicated `Qwen35Renderer`/`Qwen35Parser`. The remaining template fidelity gaps are: the `isThinking` gate on history think blocks (section 2.5), and the remaining JSON serialization bugs (Bugs A/B in section 3.1; Bug C is now fixed locally in the fork).
...
| **P0** | **JSON serialization still partially wrong in tool-using renderers** — remaining bugs: (A) shared `marshalWithSpaces` HTML-escapes `<>&`, (B) `ToolFunctionParameters` struct outputs `required` before `properties` for Qwen 3.5. Sub-bug C (`formatToolCallArgument` compact separators) is now fixed in the fork's Qwen-local path. Empirically verified against `transformers` [v5.2.0](https://github.com/huggingface/transformers/blob/v5.2.0/src/transformers/utils/chat_template_utils.py#L463-L466). | Remaining distribution shift on tool definitions and nested object key ordering. The local Qwen-family tool call argument path is improved; global/shared work remains. | **Fork**: partially fixed. **llama.cpp**: correct for JSON serialization (executes Jinja2 directly), but has its own `tojson` float precision flaw at `value.cpp:1290` (6 significant digits vs Python's ~17; `1.0` → `1`) | 3.1, 2.1 |
...
|-------|--------|-----|---------|
| **JSON serialization mismatches** in tool serialization — remaining issues are shared HTML escaping across the full tool-definition serialization path and key ordering in `ToolFunctionParameters`. The fork has already fixed compact separators and Qwen-local HTML escaping in `formatToolCallArgument`, but tool definitions still differ from HuggingFace's `tojson` override. The current callers (`qwen3.5`, `qwen3-vl`, `olmo3`) are verified as correct targets for the escaping fix, but failed implementation testing showed the work cannot stop at `marshalWithSpaces`; nested custom marshalers also need to be covered. | Remaining distribution shift on tool definitions and nested object ordering. | Implement a verified no-escape serialization path for tool definitions end-to-end and solve key ordering separately. | 3.1, 3.4, 2.1 |
