...
Qwen3-Coder reference: `Qwen/Qwen3-Coder-30B-A3B-Instruct` Jinja2 template ([verified via public access](https://huggingface.co/Qwen/Qwen3-Coder-30B-A3B-Instruct)) — confirms XML tool definitions and system-first ordering. **Important nuance:** `Qwen3CoderRenderer` is directionally correct but not fully byte-faithful. Two remaining fidelity gaps are now confirmed from the official template: `renderAdditionalKeys()` should use the template's `render_extra_keys(... | tojson | safe)` behavior for nested map/list values, and multi-type `type` fields are rendered with Python string formatting in the template rather than JSON serialization.
...
| JSON tool serialization (remaining key ordering bug; local tool-call and shared tool-definition escaping fixed) | The fork now fixes both the Qwen-local `formatToolCallArgument` path and the shared tool-definition HTML-escaping path end-to-end: structured tool call arguments render with spaced JSON and literal `<>&`, and tool definitions now also emit literal `<>&` through the full serialization graph. Remaining open issue is `ToolFunctionParameters` key ordering. Upstream still has the full original bug set. | Upstream still has the full bug; fork now has only the key-ordering gap left | **Fork** |
...
### 3.1 JSON serialization mismatches — shared HTML escaping and Sub-bug C fixed in the fork; key ordering still remains

**Fault: Both upstream Ollama (`82848a78`) and, historically, the fork. Affects every model that uses `tojson` in its training template — not just Qwen 3.5. The fork has now fixed the shared HTML-escaping path and the Qwen-local structured-argument spacing path; key ordering remains open.**
...
**Verification update (2026-03-11):** This fix is now **implemented** in the fork. Official HuggingFace templates downloaded for `Qwen/Qwen3.5-*`, `Qwen/Qwen3-VL-*`, `allenai/Olmo-3-7B-Instruct`, and `allenai/Olmo-3.1-32B-Instruct` all use `tools | tojson` for tool definitions, so those paths should emit literal `<>&`, not HTML-escaped `\u003c\u003e\u0026`, for `qwen3.5`, `qwen3-vl`, and `olmo3`. The important scope correction from failed testing on 2026-03-11 was applied: changing `marshalWithSpaces` alone was insufficient, so the final patch also covered the nested custom `MarshalJSON` paths in `api/types.go` and `internal/orderedmap`. Renderer regression tests were added for top-level and nested tool-definition strings, and the full `go test ./...` suite passed.
...
HuggingFace's `tojson` uses Python's default separators `(', ', ': ')` — JSON with spaces after `:` and `,`. The existing `marshalWithSpaces` function at `json.go:8-45` handled this for tool **definitions** in `Qwen35Renderer`, and the fork now applies the same spacing convention with HTML escaping disabled in both places that mattered for Qwen 3.5 correctness: the shared tool-definition serializer path and `formatToolCallArgument` for structured tool call arguments.
...
1. **Tool-definition escaping path, starting at `marshalWithSpaces`**: DONE in the fork on 2026-03-11. The final patch replaced the inner `json.Marshal` call with a no-HTML-escape encoder path and also covered the nested tool-related custom `MarshalJSON` implementations in `api/types.go` / `internal/orderedmap`, so nested property descriptions now stop HTML-escaping too. The spacing logic already handled Bug C for the top-level helper path. For Bug B, the `Required` and `Properties` field declarations in `api/types.go:486-491` remain a separate question.
...
| **P0** | **JSON serialization still partially wrong in tool-using renderers** — remaining bug: (B) `ToolFunctionParameters` struct outputs `required` before `properties` for Qwen 3.5. Sub-bug A (shared HTML escaping) and Sub-bug C (`formatToolCallArgument` compact separators) are now fixed in the fork. Empirically verified against `transformers` [v5.2.0](https://github.com/huggingface/transformers/blob/v5.2.0/src/transformers/utils/chat_template_utils.py#L463-L466). | Remaining distribution shift is now tool-definition key ordering. | **Fork**: partially fixed, with key ordering still open. **llama.cpp**: correct for JSON serialization (executes Jinja2 directly), but has its own `tojson` float precision flaw at `value.cpp:1290` (6 significant digits vs Python's ~17; `1.0` → `1`) | 3.1, 2.1 |
...
|-------|--------|-----|---------|
| **JSON serialization mismatches** in tool serialization — remaining issue is key ordering in `ToolFunctionParameters`. The fork has fixed compact separators, Qwen-local HTML escaping in `formatToolCallArgument`, and the shared tool-definition HTML-escaping path end-to-end. | Remaining distribution shift on tool-definition key ordering. | Solve key ordering separately. | 3.1, 3.4, 2.1 |
