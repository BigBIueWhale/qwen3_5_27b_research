...
| Tool call parser robustness | Free-form streaming parser (`parsers/qwen3coder.go`) — accepts any model output, parses after the fact. Malformed tool calls (hallucinated function names, type mismatches, unclosed `<tool_call>` tags) silently propagate as invalid `api.ToolCall` structs | Same (identical parser code) | Neither. **llama.cpp is structurally better** — PEG grammar-constrained parser (`chat.cpp:1555-1616`) masks model logits during generation so only syntactically valid tool calls with declared function names and schema-conforming parameter values can be produced. See `fork_vs_latest_upstream_and_llama_cpp.md` Section 6.6. |
