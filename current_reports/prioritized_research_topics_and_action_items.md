...
| **Informational** | Tool call parser is free-form (no grammar constraints) — malformed model output silently propagates as invalid tool calls; llama.cpp uses grammar-constrained PEG parser that prevents this structurally — see `fork_vs_latest_upstream_and_llama_cpp.md` Section 6.6 | Very High (architectural) | Structural robustness gap vs llama.cpp for agentic tool-calling workflows |
...
Beyond JSON serialization, there is a deeper **structural** difference between Ollama and llama.cpp in how tool calls are parsed. Ollama uses a free-form streaming parser (`parsers/qwen3coder.go`) that accepts whatever the model generates and tries to extract tool calls after the fact. If the model generates a malformed tool call — a hallucinated function name, an unclosed `<tool_call>` tag, a parameter value that doesn't match its declared type — the parser either silently produces an invalid `api.ToolCall` struct that propagates to the client, or loses content in its internal buffer. llama.cpp uses a grammar-constrained PEG parser (`chat.cpp:1555-1616`) that masks model logits during generation so only syntactically valid tool calls with declared function names and schema-conforming parameter values can be produced. Malformed tool calls are structurally impossible. In agentic workflows where tool call results feed back into the conversation, a single malformed tool call from Ollama's parser can cascade into corrupted history and degraded model behavior on subsequent turns. See `fork_vs_latest_upstream_and_llama_cpp.md` Section 6.6 for the detailed analysis.
