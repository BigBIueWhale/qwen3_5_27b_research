...
| **CRITICAL** | JSON serialization mismatch fix (remaining sub-bugs: shared HTML escaping + key ordering) — see Section A.1. **Update 2026-03-10:** Sub-bug C is now DONE in the fork: `formatToolCallArgument` for Qwen-family tool call arguments now emits spaced JSON with literal `<>&`, eliminating the local compact-separator bug and the Qwen-local HTML-escaping bug for tool call arguments. Remaining open scope is shared `marshalWithSpaces` callers plus `ToolFunctionParameters` key ordering. | **Medium** (shared infrastructure, per-model verification still required) | KV cache round-trip correctness (performance) + tool calling accuracy (quality) for multi-turn agentic workflows |
...
*Shared infrastructure concern:* `marshalWithSpaces` is called by the Olmo3, Qwen 3.5, and Qwen3VL renderers. Changing its internals propagates to all of them. This verification has now been completed for the current callers: official HuggingFace templates downloaded into `/tmp/olmo3-check/` and the existing Qwen template validation show that `qwen3.5`, `qwen3-vl`, and `olmo3` all use `tools | tojson` for tool definitions, so disabling HTML escaping is **correct in principle** for those paths. However, a failed implementation attempt on 2026-03-11 exposed an important scope correction: this is **not** just a `marshalWithSpaces` patch. Nested tool-definition fields still pass through custom `MarshalJSON` boundaries in `api/types.go` (`ToolPropertiesMap`, `ToolCallFunctionArguments`, `PropertyType`, and related tool-type serialization), so changing the top-level helper alone leaves nested strings HTML-escaped. The correct implementation is therefore a **full tool-definition serialization-path fix for escaping**, not a helper-only change. That shared fix is still **not implemented** in the fork as of 2026-03-11. Additionally, `formatToolCallArgument` in `qwen3coder.go` (shared by Qwen3Coder, Qwen 3.5, and Qwen3VL) called raw `json.Marshal` directly — it also needed the HTML escaping fix, but that Qwen-family-only path has already been fixed.
...
Decide upfront: fix only Qwen 3.5 (safest, narrowest blast radius), fix all Qwen family models (medium, since they share `tojson` conventions), or fix globally (maximum impact but requires completing Research topic 2 for all models). The sequencing guidance was correct: Sub-bug C first (Qwen-only code, improves KV cache round-trip) is now DONE in the fork; Sub-bug A second (shared function but likely universally correct) and Sub-bug B last (shared struct, requires universal verification or renderer-local approach) remain open.
