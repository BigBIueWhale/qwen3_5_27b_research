...
*Shared infrastructure concern:* `marshalWithSpaces` is called by the Olmo3, Qwen 3.5, and Qwen3VL renderers. Changing its internals propagates to all of them. This verification has now been completed for the current callers: official HuggingFace templates downloaded into `/tmp/olmo3-check/` and the existing Qwen template validation show that `qwen3.5`, `qwen3-vl`, and `olmo3` all use `tools | tojson` for tool definitions, so disabling HTML escaping in `marshalWithSpaces` is **correct in principle** for those paths. That shared fix is still **not implemented** in the fork as of 2026-03-11; the remaining caution is implementation scope, not template uncertainty for those current callers. Additionally, `formatToolCallArgument` in `qwen3coder.go` (shared by Qwen3Coder, Qwen 3.5, and Qwen3VL) calls raw `json.Marshal` directly — it also needed the HTML escaping fix, but that Qwen-family-only path has already been fixed.
