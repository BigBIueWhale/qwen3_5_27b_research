...
| **CRITICAL** | JSON serialization mismatch fix (remaining sub-bug: key ordering) — see Section A.1. **Update 2026-03-11:** Sub-bug A is now DONE in the fork: the full tool-definition serialization path now emits literal `<>&` end-to-end, including nested tool-type `MarshalJSON` boundaries. **Update 2026-03-10:** Sub-bug C is also DONE: `formatToolCallArgument` for Qwen-family tool call arguments emits spaced JSON with literal `<>&`. Remaining open scope is `ToolFunctionParameters` key ordering. For Qwen 3.5 specifically, success should mean the full round-trip consistency test in [`server/qwen35_kvcache_roundtrip_test.go`](/home/user/shared_vm/ollama/server/qwen35_kvcache_roundtrip_test.go) passes completely. | **Medium** (remaining work is shared ordering research / renderer-local ordering strategy) | KV cache round-trip correctness (performance) + tool calling accuracy (quality) for multi-turn agentic workflows |
...
For each renderer that handles tool calls (Qwen3Coder, Qwen 3.5, Qwen3VL, DeepSeek, GLM, Cogito, Olmo3, Nemotron), trace the complete round-trip: model output format → parser extraction → Go type storage → renderer re-serialization → prompt bytes. Prove that after the proposed fix, the re-serialized bytes exactly match what the model was trained to generate. A mismatch in ANY leg of this chain creates a KV cache miss. Document edge cases: nested objects, arrays, strings containing `<>&"`, unicode, empty objects, null values. For Qwen 3.5 specifically, success should mean the full round-trip consistency test in [`server/qwen35_kvcache_roundtrip_test.go`](/home/user/shared_vm/ollama/server/qwen35_kvcache_roundtrip_test.go) passes completely.
